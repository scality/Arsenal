...
describe('MongoClientInterface:delObject', () => {
    ...
    it('deleteObjectVerMaster:: should create phd master when no master found', done => {
        const collection = {
            updateOne: (filter, update, params, cb) => cb(null, { modifiedCount: 0 }),
        };
        const createPHDMasterStub = sinon.stub(client, 'createPHDMasterWithVersionMD').callsArg(6);
        ...
        client.deleteObjectVerMaster(collection, 'example-bucket', 'example-object', {}, logger, err => {
            assert.ifError(err);
            assert(createPHDMasterStub.calledOnce);
            return done();
        });
    });
    ...
    it('deleteObjectVerMaster:: should fail when deleteOrRepairPHD fails', done => {
        const collection = {
            updateOne: (filter, update, params, cb) => cb(null, { modifiedCount: 1 }),
        };
        sinon.stub(client, 'internalDeleteObject').callsArg(5);
