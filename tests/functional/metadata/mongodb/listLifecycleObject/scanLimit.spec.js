...
const { DelimiterNonCurrent } = require('../../../../../lib/algos/list/delimiterNonCurrent');
...
function listAllPages(metadata, baseParams, nextParams, cb) {
    ...
    const listPage = params =>
        metadata.listLifecycleObject(BUCKET_NAME, params, logger, (err, data) => {
            if (err) {
                return cb(err);
            }
            ++pages;
                ...
                return listAllBudgets(metadata, baseParams, nextParams, (err, unbounded, bounded) => {
                    ...
                    return assertLater(done, () => {
                        ...
                        SCAN_BUDGETS.forEach(budget => {
                            ...
                            assert.ok(res.pages < MAX_PAGES, `scan budget ${budget}: listing did not terminate`);
                            ...
                            assert.deepStrictEqual(
                                nonCurrentIds(res.contents).sort(),
                                expected,
