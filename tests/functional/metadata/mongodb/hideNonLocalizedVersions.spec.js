...
describe('MongoClientInterface::hideNonLocalizedVersions', () => {
    ...
    variations.forEach(variation => {
        describe(`vFormat : ${variation.vFormat}`, () => {
             ...
             * @return {undefined}
             */
            function copyVersionOverMaster(objName, versionId, cb) {
                const c = metadata.client.getCollection(BUCKET_NAME);
                return c
                    .findOne({ _id: formatVersionKey(objName, versionId, variation.vFormat) })
                    .then(doc =>
                        c.updateOne(
                            { _id: formatMasterKey(objName, variation.vFormat) },
                            { $set: { value: doc.value } },
                        ),
                    )
                    .then(() => cb())
            ...
            if (variation.vFormat === BucketVersioningKeyFormat.v1) {
                it('should resolve a listed PHD key against the localized versions only', done => {
                    let phdLocalizedVersionId;
                    async.series(
                        [
                            next =>
                                putLocalizedVersion('pfx-phd', null, (err, versionId) => {
                                    phdLocalizedVersionId = versionId;
                                    return next(err);
                                }),
                            next => putNonLocalizedVersion('pfx-phd', variation.vFormat, next),
                            next =>
                                metadata.client
                                    .getCollection(BUCKET_NAME)
                                    .updateOne(
            ...
            it('should resolve a non-localized master to the newest localized version', done => {
                async.series(
                    [
                        next => copyVersionOverMaster('pfx-mixed', mixedNonLocalizedVersionId, next),
                        next =>
                            metadata.client.getObject(
                                BUCKET_NAME,
                                'pfx-mixed',
            ...
            it('should return a non-localized master when the flag is not set', done => {
                async.series(
                    [
                        next => copyVersionOverMaster('pfx-mixed', mixedNonLocalizedVersionId, next),
                        next =>
                            metadata.client.getObject(BUCKET_NAME, 'pfx-mixed', {}, logger, (err, data) => {
                                if (err) {
                                    return next(err);
                                }
            ...
            it('should resolve an absent master against the localized versions only', done => {
                metadata.client.getObject(BUCKET_NAME, 'pfx-nonlocalized', {}, logger, err => {
                    assert(err?.is.NoSuchKey);
                    return done();
                });
            });
