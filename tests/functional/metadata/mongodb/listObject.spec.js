...
describe('MongoClientInterface::metadata.listObject', () => {
    ...
    beforeEach(done => {
        ...
        async.series([
            ...
            next => {
                ...
                putBulkObjectVersions(BUCKET_NAME, objVal.key, objVal, versionParams, 5, next);
            },
            next => {
                const objVal = {
    ...
    it('Should return empty results when bucket non existent (master)', done => {
    ...
    it('Should list versions of objects that start with prefix', done => {
        const params = {
            listingType: 'DelimiterVersions',
            maxKeys: 100,
            prefix: 'pfx2',
    ...
    it('should check entire list with pagination (version)', done => {
        const versionsPerKey = {};
        const get = (maxKeys, keyMarker, versionIdMarker, cb) => metadata.listObject(BUCKET_NAME, {
            listingType: 'DelimiterVersions',
            maxKeys,
            keyMarker,
            versionIdMarker,
