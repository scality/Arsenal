...
class IndexTransaction {
     ...
     *                    {
     *                      notExists: the object key
     *                    }
     *
     * @throws {Error} an error described by the following properties
     *                 - missingCondition if the condition is empty
     *                 - pushOnCommittedTransaction if already committed
     *                 - unsupportedConditionalOperation if the condition is not 'notExists'
     *                 - missingKey if the key is missing from the op
     *
     * @returns {undefined}
     */
    addCondition(condition) {
        ...
        if ('notExists' in condition === false) {
            throw propError('unsupportedConditionalOperation', 'unsupported conditional operation');
        }
        if (condition.notExists === undefined) {
            throw propError('missingKey', 'missing key for conditional operation');
        }
        this.operations.push(condition);
    }
