...
const backend = {
    ...
    head: function headMem(objectGetInfos, reqUids, callback) {
        let key;
        if (Array.isArray(objectGetInfos)) {
            key = objectGetInfos[0].key ? objectGetInfos[0].key : objectGetInfos[0];
        } else {
            key = objectGetInfos.key ? objectGetInfos.key : objectGetInfos;
        }
        process.nextTick(() => {
            if (!ds[key]) {
                return callback(errors.NoSuchKey);
            }
            return callback(null, { keyContext: ds[key].keyContext, lastModified: new Date() });
        });
    },
};
