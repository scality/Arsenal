...
class DataWrapper {
    ...
    destLocationConstraintName, dataLocator, dataStoreContext, callback) {
        const serverSideEncryption = destBucketMD.getServerSideEncryption();
    ...
    abortMPU(objectKey, uploadId, location, bucketName, log, callback) {
        if (this.config.backends.data === 'multiple') {
    ...
    contentSettings, lcCheckFn, log, callback) {
    ...
    listParts(mpuInfo, log, callback) {
        const {
            objectKey,
            uploadId,
            bucketName,
