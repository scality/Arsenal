...
const BackendInfo = require('../../models/BackendInfo').default;
...
class DataWrapper {
    constructor(client, implName, config, kms, metadata, locStorageCheckFn, vault) {
        this.client = client;
            ...
            return this._put(
                ...
                (err, dataRetrievalInfo, hashedStream) => {
                    if (err) {
                        ...
                        return this.locStorageCheckFn(location, -valueSize, log, error => {
                            if (error) {
    ...
    get(objectGetInfo, response, log, cb) {
        ...
        const isRequiredStringKey = constants.clientsRequireStringKey[this.implName];
        ...
        log.debug('sending get to datastore', { implName: this.implName, key, range, method: 'get' });
        ...
        this.client.get(clientGetInfo, range, log.getSerializedUids(), (err, stream) => {
            if (err) {
                log.error('get error from datastore', { error: err, implName: this.implName });
                return cb(errors.ServiceUnavailable);
            }
            // kms decipher bundle stream prepared by cloudserver
            // objectGet function to return kms error before data handling
            if (objectGetInfo.decipherStream) {
            ...
            if (objectGetInfo.cipheredDataKey) {
                ...
                return this.kms.createDecipherBundle(serverSideEncryption, offset, log, (err, decipherBundle) => {
                    if (err) {
                        log.error('cannot get decipher bundle from kms', { method: 'data.wrapper.data.get' });
                        return cb(err);
                    }
    ...
    delete(objectGetInfo, log, cb) {
        ...
        log.debug('sending delete to datastore', { implName: this.implName, key, method: 'delete' });
        // If the key is explicitly set to null, the part to
        ...
        this._retryDelete(clientGetInfo, log, 0, err => {
            if (err && !err.is.ObjNotFound) {
                log.error('delete error from datastore', { error: err, key: objectGetInfo.key, moreRetries: 'no' });
            }
            if (!err) {
                // pass size as negative so location metric is decremented
                return this.locStorageCheckFn(objectGetInfo.dataStoreName, -objectGetInfo.size, log, err => {
                    if (err) {
    ...
    batchDelete(locations, requestMethod, newObjDataStoreName, log, cb) {
        ...
        if (this._shouldSkipDelete(locations, requestMethod, newObjDataStoreName)) {
            return process.nextTick(cb);
        }
        const delLog = this.logger.newRequestLoggerFromSerializedUids(log.getSerializedUids());
        delLog.debug('initiating batch delete', {
            keys: locations,
    ...
    checkHealth(log, cb, flightCheckOnStartUp) {
        ...
        return this.client.healthcheck(flightCheckOnStartUp, log, (err, result) => {
            let respBody = {};
            if (err) {
    ...
    getDiskUsage(log, cb) {
        if (!this.client.getDiskUsage) {
            log.debug('returning empty disk usage as fallback', { implName: this.implName });
            return cb(null, {});
        }
        return this.client.getDiskUsage(this.config, log.getSerializedUids(), cb);
    }
    ...
    copyObject(
        ...
        if (
            ...
            backendUtils.externalBackendCopy(
                this.config,
                sourceLocationConstraintName,
            ...
            const externalSourceVersionId = objectGetInfo.dataStoreVersionId;
            return this.client.copyObject(
                request,
                destLocationConstraintName,
                externalSourceKey,
                externalSourceVersionId,
                sourceLocationConstraintName,
                storeMetadataParams,
                this.config,
                log,
        ...
        return async.mapLimit(
            ...
            function copyPart(part, copyCb) {
                if (part.dataStoreType === 'azure') {
                    ...
                    return async.parallel(
                        [
                            parallelCb => that.get(part, passThrough, log, err => parallelCb(err)),
                            parallelCb =>
                                that._dataCopyPut(
                                    serverSideEncryption,
                                    passThrough,
            ...
            (err, results) => {
                if (err) {
                    log.debug('error transferring data from source', { error: err });
                    return cb(err);
                }
    ...
    uploadPartCopy(
        ...
        if (dataLocator.length === 0) {
            return process.nextTick(() => {
                callback(null, constants.emptyFileMd5, lastModified, serverSideEncryption, []);
            });
        }

        // if destination mpu was initiated in legacy version
        if (destLocationConstraintName === undefined) {
            const backendInfoObj = lcCheckFn(request, null, destBucketMD, log);
        ...
        const srcType = this.config.getLocationConstraintType(sourceLocationConstraintName);
        const dstType = this.config.getLocationConstraintType(destLocationConstraintName);
        const locationTypeMatch =
            this.config.backends.data === 'multiple' && srcType === dstType && constants.hasCopyPartBackends[srcType];
        ...
        if (locationTypeMatch && dataLocator.length === 1) {
            ...
            const sourceVersionId = dataLocator[0].dataStoreVersionId;
            return this.client.uploadPartCopy(
                ...
                (error, eTag) => {
                    ...
                    return callback(doSkip, eTag, lastModified, serverSideEncryption);
                },
            );
        }

        const backendInfo = new BackendInfo(this._config, destLocationConstraintName);
        ...
        return async.forEachOfSeries(
            ...
            function copyPart(part, index, cb) {
                if (part.dataStoreType === 'azure') {
                    ...
                    return async.parallel(
                        [
                            next =>
                                that.get(part, passThrough, log, err => {
                                    if (err) {
                                        log.error('error getting data part from Azure', {
                                            error: err,
                                            method: 'objectPutCopyPart::' + 'multipleBackendGateway.copyPart',
                                        });
                                        return next(err);
                                    }
                                    return next();
                ...
                return that.get(part, null, log, (err, stream) => {
                    ...
                    const hashedStream = new RelayMD5Sum(totalHash, updatedHash => {
            ...
            err => {
                ...
                if (err && err !== skipError) {
                    log.debug('error transferring data from source', { error: err, method: 'goGetData' });
                    return callback(err);
                }
                if (totalHash) {
                ...
                if (err && err === skipError) {
                    return callback(skipError, totalHash, lastModified, serverSideEncryption);
                }
                return callback(null, totalHash, lastModified, serverSideEncryption, locations);
            },
        );
    }

    abortMPU(objectKey, uploadId, location, bucketName, request, destBucket, lcCheckFn, log, callback) {
        if (this.config.backends.data === 'multiple') {
            ...
            if (location === undefined) {
                const backendInfoObj = lcCheckFn(request, null, destBucket, log);
                if (backendInfoObj.err) {
                    return process.nextTick(() => {
                        callback(backendInfoObj.err);
                    });
            ...
            return this.client.abortMPU(objectKey, uploadId, location, bucketName, log, callback);
        }
        return callback(null, false);
    }

    completeMPU(request, mpuInfo, mdInfo, location, userMetadata, contentSettings, tagging, lcCheckFn, log, callback) {
        const { objectKey, uploadId, jsonList, bucketName, destBucket } = mpuInfo;
        if (this.config.backends.data === 'multiple') {
            ...
            return this.client.completeMPU(
                objectKey,
                uploadId,
                location,
                jsonList,
        ...
        if (this.config.backends.data === 'multiple') {
            return this.client.createMPU(
                ...
                (err, dataBackendResObj) => {
                    ...
                    const configLoc = this.config.locationConstraints[locConstraint];
                    if (
                        locConstraint &&
                        configLoc &&
                        configLoc.type &&
    ...
    listParts(mpuInfo, request, lcCheckFn, log, callback) {
        const { objectKey, uploadId, bucketName, partNumberMarker, maxParts, mpuOverviewObj, destBucket } = mpuInfo;
        if (this.config.backends.data === 'multiple') {
            let location;
            // if controlling location constraint is not stored in object
            // metadata, mpu was initiated in legacy S3C, so need to
    ...
    putPart(request, mpuInfo, streamingV4Params, objectLocationConstraint, lcCheckFn, log, callback) {
        const { stream, destinationBucket, size, objectKey, uploadId, partNumber, bucketName } = mpuInfo;
        if (this.config.backends.data === 'multiple') {
            // if mpu was initiated in legacy version
            if (objectLocationConstraint === undefined) {
                const backendInfoObj = lcCheckFn(request, null, destinationBucket, log);
                if (backendInfoObj.err) {
                    return process.nextTick(() => {
                        callback(backendInfoObj.err);
                    });
                ...
                (err, partInfo) => {
                    if (err) {
                        log.error('error putting part to data backend', {
                            error: err,
                            method: 'objectPutPart::multipleBackendGateway.uploadPart',
                        });
                        return callback(err);
                    }
    ...
    objectTagging(method, objectKey, bucket, objectMD, log, callback) {
        if (this.config.backends.data === 'multiple') {
            return this.client.objectTagging(method, objectKey, bucket, objectMD, log, err => callback(err));
        }
        return callback();
    }
    ...
    toObjectGetInfo(objectKey, bucketName, storageLocation) {
        if (this.client.toObjectGetInfo) {
            return this.client.toObjectGetInfo(objectKey, bucketName, storageLocation);
        }
        return null;
    }

    protectAzureBlocks(bucketName, objectKey, objGetInfo, log, callback) {
        if (objGetInfo && objGetInfo[0] && this.config.backends.data === 'multiple') {
            return this.client.protectAzureBlocks(bucketName, objectKey, objGetInfo[0].dataStoreName, log, err => {
                if (err) {
                    return callback(err.customizeDescription('Error ' + `deleting object on Azure: ${err.message}`));
                }
                return callback();
            });
        }
     ...
     * @returns {function} cb - callback
     */
    _putForCopy(cipherBundle, stream, part, dataStoreContext, destBackendInfo, log, cb) {
        return this.put(
            cipherBundle,
            stream,
            part.size,
            ...
            (error, partRetrievalInfo) => {
                ...
                const partResult = {
                    ...
                    dataStoreType: partRetrievalInfo.dataStoreType,
                    start: part.start,
                    size: part.size,
                };
                if (cipherBundle) {
                ...
                if (partRetrievalInfo.dataStoreVersionId) {
                    partResult.dataStoreVersionId = partRetrievalInfo.dataStoreVersionId;
                }
     ...
     * @returns {function} cb - callback
     */
    _dataCopyPut(serverSideEncryption, stream, part, dataStoreContext, destBackendInfo, log, cb) {
        if (serverSideEncryption) {
            return this.kms.createCipherBundle(serverSideEncryption, log, (err, cipherBundle) => {
                if (err) {
                    log.debug('error getting cipherBundle');
                    return cb(err);
                }
                return this._putForCopy(cipherBundle, stream, part, dataStoreContext, destBackendInfo, log, cb);
            });
        }
        // Copied object is not encrypted so just put it
        // without a cipherBundle
        return this._putForCopy(null, stream, part, dataStoreContext, destBackendInfo, log, cb);
    }
    ...
    _dataCopyPutPart(
        ...
        const destLocationConstraintName = destBackendInfo.getControllingLocationConstraint();
        if (externalBackends[this.config.locationConstraints[destLocationConstraintName].type]) {
            return this.client.uploadPart(
                null,
                null,
                stream,
                ...
                (err, partInfo) => {
                    if (err) {
                        log.error('error putting part to AWS', {
                            error: err,
                            method: 'objectPutCopyPart::' + 'multipleBackendGateway.uploadPart',
                        });
                        return cb(errors.ServiceUnavailable);
                    }
        ...
        if (serverSideEncryption) {
            return this.kms.createCipherBundle(
                serverSideEncryption,
                log,
                (err, cipherBundle) => {
                    if (err) {
    ...
    _put(cipherBundle, value, valueSize, keyContext, backendInfo, log, cb) {
        ...
        if (value) {
            ...
            value.once('clientError', err => {
                log.debug('destroying hashed stream', { err });
        ...
        if (this.implName === 'multipleBackends') {
            ...
            return this.client.put(
                ...
                (err, dataRetrievalInfo) => {
                    if (err) {
                        log.error('put error from datastore', { error: err, implName: this.implName });
                        if (err.httpCode === 408) {
        ...
        return this.client.put(writeStream, valueSize, keyContext, log.getSerializedUids(), (err, key) => {
            if (err) {
                log.error('put error from datastore', { error: err, implName: this.implName });
                if (err.httpCode === 408) {
                    return cb(errors.IncompleteBody);
    ...
    _retryDelete(objectGetInfo, log, count, cb) {
        ...
        return this.client.delete(objectGetInfo, log.getSerializedUids(), err => {
            if (err) {
    ...
    _shouldSkipDelete(locations, requestMethod, newObjDataStoreName) {
        ...
        if (!Array.isArray(locations) || !locations[0] || !locations[0].dataStoreType) {
            return false;
        }
        const isSkipBackend = externalBackends[locations[0].dataStoreType];
        const isMatchingBackends = locations[0].dataStoreName === newObjDataStoreName;
