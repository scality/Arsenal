            ...
            function copyPart(part, copyCb) {
                if (part.dataStoreType === 'azure') {
                    ...
                    return async.parallel([
                        parallelCb => this.get(part, passThrough, log, err =>
                            parallelCb(err)),
                        parallelCb => this._dataCopyPut(serverSideEncryption,
                ...
                return this.get(part, null, log, (err, stream) => {
                    if (err) {
                        return copyCb(err);
                    }
                    return this._dataCopyPut(serverSideEncryption, stream,
        ...
        return async.forEachOfSeries(dataLocator,
            function copyPart(part, index, cb) {
                if (part.dataStoreType === 'azure') {
                    const passThrough = new PassThrough();
                    return async.parallel([
                        next => this.get(part, passThrough, log, err => {
                            if (err) {
                        ...
                        next => this._dataCopyPutPart(request,
                ...
                return this.get(part, null, log, (err, stream) => {
                    ...
                    return this._dataCopyPutPart(request, serverSideEncryption,
