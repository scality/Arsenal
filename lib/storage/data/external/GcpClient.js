...
const { createLogger, logHelper, removeQuotes } = require('./utils');

const missingVerIdInternalError = errorInstances.InternalError.customizeDescription(
    'Invalid state. Please ensure versioning is enabled ' + 'in GCP for the location constraint and try again.',
);
...
class GcpClient extends AwsClient {
    ...
    constructor(config) {
        ...
        this._gcpParams = {
            s3Params: this._s3Params,
            bucketName: this._gcpBucketName,
    ...
    put(stream, size, keyContext, reqUids, callback) {
        if (keyContext.isDeleteMarker) {
            return this._putDeleteMarker(keyContext, reqUids, callback);
        }
        return super.put(stream, size, keyContext, reqUids, callback);
    }

    // a delete marker deletes the live version on the backend,
    // best-effort: NoSuchKey means a previous marker already removed it.
    // GCS returns no generation on delete, so the marker stays
    // metadata-only with no backend version
    _putDeleteMarker(keyContext, reqUids, callback) {
        const gcpKey = this._createGcpKey(keyContext.bucketName, keyContext.objectKey, this._bucketMatch);
    ...
    // tags are stored in and served from the S3 metadata layer: do not
    // propagate them to the backend, as GCS has no object tagging API
    // (the inherited AWS implementation would fail with NotImplemented)
    // and emulating it with a metadata-rewriting copy would create a new
    // generation, desynchronizing the stored dataStoreVersionId
    ...
    healthcheck(location, callback) {
        ...
        async.parallel(
            ...
            (err, result) => {
                if (err) {
                    return callback(
                        errorInstances.InternalFailure.customizeDescription('Unable to perform health check'),
                    );
                }

                gcpResp[location] = result.main.error || result.mpu.error ? { error: true, external: true } : {};
    ...
    createMPU(
        ...
        return this._client.createMultipartUpload(params, (err, mpuResObj) => {
            if (err) {
                ...
                return callback(
                    errorInstances.ServiceUnavailable.customizeDescription(
                        'Error returned from ' + `GCP: ${err.message}`,
                    ),
                );
            }
    ...
    completeMPU(jsonList, mdInfo, key, uploadId, bucketName, log, callback) {
        ...
        return this._client.completeMultipartUpload(mpuParams, (err, completeMpuRes) => {
            if (err) {
                logHelper(
                    log,
                    'error',
            ...
            completeObjData.contentLength = Number.parseInt(completeMpuRes.ContentLength, 10);
    ...
    uploadPart(request, streamingV4Params, stream, size, key, uploadId, partNumber, bucketName, log, callback) {
        ...
        if (request) {
            const partStream = prepareStream(request, streamingV4Params, this._vault, log, callback);
    ...
    uploadPartCopy(request, gcpSourceKey, sourceVersionId, sourceLocationConstraintName, config, log, callback) {
        const destBucketName = request.bucketName;
        const destObjectKey = request.objectKey;
        const destGcpKey = this._createGcpKey(destBucketName, destObjectKey, this._bucketMatch);

        const sourceGcpBucketName = config.getGcpBucketNames(sourceLocationConstraintName).bucketName;
        ...
        if (copySourceRange) {
            return callback(
                errorInstances.NotImplemented.customizeDescription(
                    'Error returned from ' + `${this.clientType}: copySourceRange not implemented`,
                ),
            );
        }

        // the versionId suffix is translated into the GCS source
        // generation header by the GcpService middleware
        const copySourceVersion = sourceVersionId ? `?versionId=${encodeURIComponent(sourceVersionId)}` : '';
        const params = {
            Bucket: this._mpuBucketName,
            CopySource: `${sourceGcpBucketName}/${gcpSourceKey}${copySourceVersion}`,
            Key: destGcpKey,
            UploadId: uploadId,
            PartNumber: partNumber,
        };
            ...
            if (err) {
                if (err.code === 'AccesssDenied') {
                    logHelper(
                        log,
                        'error',
                        'Unable to access ' + `${sourceGcpBucketName} GCP bucket`,
                        err,
                        this._dataStoreName,
                        this.clientType,
                    );
                    return callback(
                        errorInstances.AccessDenied.customizeDescription(
                            'Error: Unable to access ' + `${sourceGcpBucketName} GCP bucket`,
                        ),
                    );
                }
                logHelper(log, 'error', 'error from data backend on ' + 'uploadPartCopy', err, this._dataStoreName);
                return callback(
                    errorInstances.ServiceUnavailable.customizeDescription(
                        'Error returned from ' + `GCP: ${err.message}`,
                    ),
        ...
        return this._client.abortMultipartUpload(getParams, err => {
            if (err) {
                logHelper(
                    log,
                    'error',
                    'err from data backend ' + 'on abortMPU',
                    err,
                    this._dataStoreName,
                    this.clientType,
                );
