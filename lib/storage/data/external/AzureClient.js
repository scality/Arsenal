...
class AzureClient {
     ...
     * @param {result} [resultCb] Command result processor
     ...
     * @returns {Promise<any>}
     *
     * @callback ErrorWrapper~ResultCb
     * @param {azure.RestError} [restErr] Error returned by Azure SDK command
     ...
     * @returns {void}
     */
    _errorWrapper(s3Method, azureMethod, command, resultCb, log, cb) {
        if (log) {
            log.info(`calling azure ${azureMethod}`);
        }
        try {
            command(this._client).then(
                result => resultCb(null, result),
                resultCb,
            );
        } catch (err) {
    ...
    put(stream, size, keyContext, reqUids, callback, skey, metadata) {
        ...
        this.protectAzureBlocks(metadata, keyContext.bucketName,
            ...
            // if error returned, there is ongoing MPU, so do not put
                ...
                if (size === 0) {
                    return this._errorWrapper('put', 'uploadData',
                        client => client.getBlockBlobClient(azureKey).upload('', 0, options),
                        err => {
                            if (err) {
                ...
                return this._errorWrapper('put', 'createBlockBlobFromStream',
                    client => client.getBlockBlobClient(azureKey).upload(() => stream, size, options),
                    err => {
                        if (err) {
                            logHelper(log, 'error', 'err from Azure PUT data ' +
                        'backend', err, this._dataStoreName);
    ...
    head(objectGetInfo, reqUids, callback) {
        ...
        return this._errorWrapper('head', 'getBlobProperties',
            client => client.getBlockBlobClient(key).getProperties(),
            (err, data) => {
                if (err) {
                    let logLevel;
                    let retError;
    ...
    delete(objectGetInfo, reqUids, callback) {
        ...
        return this._errorWrapper('delete', 'deleteBlobIfExists',
            client => client.getBlockBlobClient(key).deleteIfExists(options),
            err => {
    ...
    healthcheck(location, callback, flightCheckOnStartUp) {
        const azureResp = {};
        const healthCheck = flightCheckOnStartUp ?
            client => client.createIfNotExists() :
            client => client.exists();
        this._errorWrapper('healthcheck', 'checkAzureHealth', healthCheck,
            err => {
    ...
    completeMPU(jsonList, mdInfo, key, uploadId, bucket, metaHeaders,
        ...
        return this._errorWrapper('completeMPU', 'commitBlocks',
            client => client.getBlockBlobClient(azureKey).commitBlockList(commitList, options),
            err => {
                if (err) {
    ...
    objectPutTagging(key, bucket, objectMD, log, callback) {
        ...
        this._errorWrapper('objectPutTagging', 'setBlobMetadata',
            client => client.getBlockBlobClient(azureKey).setMetadata(azureMD),
            err => {
                if (err) {
    ...
    copyObject(request, destLocationConstraintName, sourceKey,
        ...
        this._errorWrapper('copyObject', 'startCopyBlob',
            client => client.getBlockBlobClient(destAzureKey).beginCopyFromURL(
                `${this._azureStorageEndpoint}${sourceContainerName}/${sourceKey}`, options),
            (err, poller) => {
                if (err) {
                ...
                const res = poller.getOperationState().result;
                if (res.copyProgress === 'pending') {
                    logHelper(log, 'error', 'Azure copy status is pending',
                        err, this._dataStoreName);
                    const copyId = res.copyId;
                    return this._errorWrapper('copyObject', 'abortCopyBlob',
                        client => client.getBlockBlobClient(destAzureKey).abortCopyFromURL(copyId),
                        err => {
                            if (err) {
