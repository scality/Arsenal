...
const { validateAndFilterMpuParts } = require('../../../s3middleware/processMpuParts');

const { createLogger, logHelper, translateAzureMetaHeaders } = require('./utils');
...
class AzureClient {
    ...
    _errorWrapper(s3Method, azureMethod, command, log, cb) {
        ...
        command(this._client).then(result => cb(null, result), cb);
    }

    _createAzureKey(requestBucketName, requestObjectKey, bucketMatch) {
        if (bucketMatch) {
    ...
    protectAzureBlocks(metadata, bucketName, objectKey, dataStoreName, log, cb) {
        ...
        return metadata.listMultipartUploads(mpuBucketName, listingParams, log, (err, mpuList) => {
            if (err && !err.NoSuchBucket) {
                log.error('Error listing MPUs for Azure delete', { error: err, dataStoreName });
                return cb(errors.ServiceUnavailable);
            }
            if (mpuList && mpuList.Uploads && mpuList.Uploads.length > 0) {
                const error = errors.MPUinProgress;
                log.error('Error: cannot put/delete object to Azure with ' + 'same key name as ongoing MPU on Azure', {
                    error,
                    dataStoreName,
                });
                return cb(error);
    ...
    _getAzureContentSettingsHeaders(properties) {
        return {
            blobContentMD5: properties.contentMD5 ? objectUtils.getMD5Buffer(properties.contentMD5) : undefined,
    ...
    put(stream, size, keyContext, reqUids, callback, skey, metadata) {
        ...
        this.protectAzureBlocks(
            metadata,
            keyContext.bucketName,
            ...
            err => {
                ...
                if (err) {
                    return callback(err.customizeDescription(`Error putting object to Azure: ${err.message}`));
                }
                const azureKey = this._createAzureKey(keyContext.bucketName, keyContext.objectKey, this._bucketMatch);
                const options = {
                    metadata: translateAzureMetaHeaders(keyContext.metaHeaders, keyContext.tagging),
                    blobHTTPHeaders: this._getAzureContentSettingsHeaders(keyContext || {}),
                };
                if (size === 0) {
                    return this._errorWrapper(
                        ...
                        async client => {
                            try {
                                ...
                                logHelper(log, 'error', 'err from Azure PUT data backend', err, this._dataStoreName);
                                throw errorInstances.ServiceUnavailable.customizeDescription(
        ...
        return this._errorWrapper(
            ...
            async client => {
                try {
                    ...
                    logHelper(log, logLevel, 'err from Azure HEAD data backend', err, this._dataStoreName);
                    throw retError;
                }
            },
            log,
    ...
    get(objectGetInfo, range, reqUids, callback) {
        ...
        if (azureStreamingOptions) {
            ...
            rangeStart =
                typeof azureStreamingOptions.rangeStart === 'string'
                    ? parseInt(azureStreamingOptions.rangeStart, 10)
                    : azureStreamingOptions.rangeStart;
            rangeEnd =
                typeof azureStreamingOptions.rangeEnd === 'string'
        ...
        this._errorWrapper(
            ...
            async client => {
                try {
                    ...
                    logHelper(log, 'error', 'err from Azure GET data backend', err, this._dataStoreName);
    ...
    delete(objectGetInfo, reqUids, callback) {
        ...
        const key = typeof objectGetInfo === 'string' ? objectGetInfo : objectGetInfo.key;
        let options;
        ...
        return this._errorWrapper(
            ...
            async client => {
                try {
                    ...
                    const log = createLogger(reqUids);
                    logHelper(log, 'error', 'error deleting object from Azure datastore', err, this._dataStoreName);
                    throw errorInstances.ServiceUnavailable.customizeDescription(
                        `Error returned from Azure: ${err.message}`,
                    );
                }
    ...
    uploadPart(request, streamingV4Params, partStream, size, key, uploadId, partNumber, bucket, log, callback) {
        const azureKey = this._createAzureKey(bucket, key, this._bucketMatch);
        const params = { bucketName: this._azureContainerName, partNumber, size, objectKey: azureKey, uploadId };
        ...
        const dataRetrievalInfo = {
            ...
            dataStoreName: this._dataStoreName,
            dataStoreType: 'azure',
            numberSubParts: azureMpuUtils.getSubPartInfo(size).expectedNumberSubParts,
        };

        if (size === 0) {
            log.debug('0-byte part does not store data', { method: 'uploadPart' });
        ...
        if (size <= azureMpuUtils.maxSubPartSize) {
            ...
            return azureMpuUtils.putSinglePart(
                errorWrapperFn,
                stream,
                params,
                this._dataStoreName,
        ...
        const errorWrapperFn = this._errorWrapper.bind(this);
        return azureMpuUtils.putSubParts(
            errorWrapperFn,
            stream,
            params,
            this._dataStoreName,
    ...
    completeMPU(jsonList, mdInfo, key, uploadId, bucket, metaHeaders, contentSettings, tagging, log, callback) {
        const azureKey = this._createAzureKey(bucket, key, this._bucketMatch);
        ...
        if (!jsonList.uncommittedBlocks) {
            ...
            filteredPartsObj = validateAndFilterMpuParts(storedParts, jsonList, mpuOverviewKey, splitter, log);
            ...
            filteredPartsObj.partList.forEach(part => {
                ...
                const subPartIds = azureMpuUtils.getSubPartIds(part.locations[0], uploadId);
        ...
        return this._errorWrapper(
            ...
            async client => {
                try {
                    ...
                    logHelper(log, 'error', 'err completing MPU on Azure datastore', err, this._dataStoreName);
                    throw errorInstances.ServiceUnavailable.customizeDescription(
                        `Error returned from Azure: ${err.message}`,
                    );
                }
        ...
        this._errorWrapper(
            ...
            async client => {
                try {
                    ...
                    logHelper(log, 'error', 'err putting object tags to Azure backend', err, this._dataStoreName);
                    throw errors.ServiceUnavailable;
                }
            },
            log,
    ...
    copyObject(
        ...
        const destAzureKey = this._createAzureKey(destContainerName, destObjectKey, this._bucketMatch);

        const sourceContainerName = config.locationConstraints[sourceLocationConstraintName].details.azureContainerName;
            ...
            async client => {
                ...
                try {
                    const poller = await client
                        .getBlockBlobClient(destAzureKey)
                        .beginCopyFromURL(`${this._azureStorageEndpoint}${sourceContainerName}/${sourceKey}`, options);
                    ...
                    if (err.code === 'CannotVerifyCopySource') {
                        // TOOD: may use a constant (or type) from SDK ??
                        logHelper(
                            log,
                            'error',
                        ...
                        throw errorInstances.AccessDenied.customizeDescription(
                            `Error: Unable to access ${sourceContainerName} Azure Container`,
                        );
                    }
                    logHelper(log, 'error', 'error from data backend on copyObject', err, this._dataStoreName);
                    throw errorInstances.ServiceUnavailable.customizeDescription(
                ...
                try {
                    ...
                    logHelper(log, 'error', 'error from data backend on abortCopyBlob', err, this._dataStoreName);
                    throw errorInstances.ServiceUnavailable.customizeDescription(
                        `Error returned from AWS on abortCopyBlob: ${err.message}`,
                    );
                }
                throw errorInstances.InvalidObjectState.customizeDescription(
