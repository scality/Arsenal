...
class AwsClient {
    ...
    put(stream, size, keyContext, reqUids, callback) {
        try {
            ...
            // AWS SDK v3 requires the stream to be in flowing or paused state, not null
            // Explicitly set up the stream to be consumed
            if (stream.readable && stream.readableFlowing === null) {
                // Calling .resume() triggers flowing mode which AWS SDK v3 can consume
                stream.resume();
                // Immediately pause to let SDK v3 control the flow
                stream.pause();
            }
    ...
    get(objectGetInfo, range, reqUids, callback) {
        ...
        this._client.send(new GetObjectCommand(params)).then(data => {
            ...
            const streamWrapper = {
                ...
                get readable() { return stream.readable; },
                get readableEnded() { return stream.readableEnded; },
                get readableFlowing() { return stream.readableFlowing; }
            }
            return callback(null, streamWrapper);
        }).catch(err => {
