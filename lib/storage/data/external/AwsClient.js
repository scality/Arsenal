    ...
    setup(cb) {
        ...
        this._client.send(new GetBucketLocationCommand({ Bucket: this._awsBucketName }))
            ...
            if (region && isAWS) {
                ...
                // Create a fresh config object with only the properties we need, ensuring endpoint is a string
                const clientParams = {
                    credentials: this._s3Params.credentials,
                    region: region,
                    endpoint: endpoint, // Explicitly set as string, not a provider function
            ...
            if (!isAWS) {
                ...
                // Don't spread _s3Params as it might contain a function endpoint
                const endpointString = this._originalEndpoint || 'http://127.0.0.1:8001';
                // Create a fresh config object with only the properties we need, ensuring endpoint is a string
                const clientParams = {
                    credentials: this._s3Params.credentials,
                    region: this._s3Params.region || 'us-east-1',
                    endpoint: endpointString, // Explicitly set as string
                    forcePathStyle: this._s3Params.forcePathStyle !== undefined ? this._s3Params.forcePathStyle : true,
                    customUserAgent: this._s3Params.customUserAgent,
                    httpOptions: this._s3Params.httpOptions,
                    maxRetries: this._s3Params.maxRetries,
