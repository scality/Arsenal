...
class AwsClient {
    constructor(config) {
        ...
        // Try multiple steps to see which one works
        ['build', 'serialize', 'finalizeRequest', 'deserialize'].forEach(step => {
            this._client.middlewareStack.add(
                (next, context) => async args => {
                    console.log(`[Middleware ${step}] Running for command:`, context.commandName);
                    
                    const request = args?.request;
                    if (request instanceof HttpRequest && request.headers) {
                        let contentLength = request.headers['content-length'] || 
                            request.headers['Content-Length'];
                        
                        if (!contentLength && context.input?.ContentLength !== undefined) {
                            contentLength = String(context.input.ContentLength);
                        }
                        
                        console.log(`[Middleware ${step}] Headers before:`, {
                            'content-length': request.headers['content-length'],
                            'Content-Length': request.headers['Content-Length'],
                            'inputContentLength': context.input?.ContentLength
                        });
                        
                        if (contentLength) {
                            request.headers['Content-Length'] = contentLength;
                            request.headers['content-length'] = contentLength;
                            request.headers['x-amz-decoded-content-length'] = contentLength;
                        }
                        
                        console.log(`[Middleware ${step}] Headers after:`, {
                            'content-length': request.headers['content-length'],
                            'Content-Length': request.headers['Content-Length']
                        });
                    }
                ...
                { 
                    step: step,
                    name: `contentLengthFixMiddleware_${step}`,
                    priority: 'high'
                }
            );
        });
        
        console.log('[AwsClient] Middleware stack size:', this._client.middlewareStack.size);
