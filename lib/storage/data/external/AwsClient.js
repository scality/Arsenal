    ...
    put(stream, size, keyContext, reqUids, callback) {
        ...
        try {
            ...
            const putCb = (err, data) => {
                ...
                // This handles cases where Upload utility doesn't include it in the response
            ...
            // AWS SDK v3 stream handling: The SDK expects streams to be immediately readable
            // MD5Sum Duplex streams from DataWrapper may not be in the right state
            ...
            // AWS SDK v3 is stricter about zero-length uploads with streams
            // For ContentLength: 0, don't set Body (stream) - SDK v3 validation issue
            if (size === 0) {
                console.error('[AwsClient.put] Zero-length upload, not setting Body/stream');
                // Don't set Body for zero-length uploads - let SDK v3 handle it
                return this._client.send(new PutObjectCommand(uploadParams)).then(res => putCb(null, res))
                    .catch(err => putCb(err));
            }

                        // Replace your current Upload section with this enhanced version:
            uploadParams.Body = stream;
            console.error('[AwsClient.put] About to upload with Upload utility:', {
            ...
            if (size < 5 * 1024 * 1024) { 
                console.error('[AwsClient.put] Using PutObjectCommand for small file');
                uploadParams.ContentLength = size;
                console.log('uploadParams for small file', uploadParams);
            ...
            // Don't force partSize for small files - let Upload decide
            // Only set partSize for very large files to control memory usage
            ...
            return upload.done().then(async res => {
                ...
                // If VersionId is missing and versioning is enabled, try to get it from HeadObject
                if (!res.VersionId && this._supportsVersioning) {
                    ...
                    try {
                        ...
                        console.error('[AwsClient.put] HeadObject response:', {
                            ...
                            headers: headResult.$metadata?.httpHeaders,
                        });
