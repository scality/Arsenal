...
class AwsClient {
    ...
    get(objectGetInfo, range, reqUids, callback) {
        ...
        this._client.send(new GetObjectCommand(params)).then(data => {
            ...
            // AWS SDK v3 returns ChecksumStream in data.Body
            ...
            if (stream && stream.source && typeof stream.source.pipe === 'function') {
                console.log('[Arsenal get] Using source IncomingMessage stream');
                stream = stream.source;
            }
            else if (stream && typeof stream.pipe !== 'function') {
                console.log('[Arsenal get] No .pipe() and no source, converting...');
                // Convert via transformToWebStream as fallback
    ...
    uploadPartCopy(request, awsSourceKey, sourceLocationConstraintName, config, log, callback) {
        ...
        return this._client.send(new CopyObjectCommand(params))
        .then(res => {
