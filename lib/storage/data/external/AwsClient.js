...
class AwsClient {
    ...
    // GCS 404s when a previous marker already removed the live version, and
    // returns no generation, so the marker stays metadata-only
    async _putDeleteMarker(params, putCb) {
        let data;
        try {
            data = await this._client.send(new DeleteObjectCommand(params));
        } catch (err) {
    ...
    put(stream, size, keyContext, reqUids, callback) {
        ...
        const putCb = (err, data) => {
            ...
            if (!dataStoreVersionId && this._supportsVersioning && !keyContext.isDeleteMarker) {
                logHelper(
                    log,
                    'error',
        ...
        if (keyContext.isDeleteMarker) {
            return this._putDeleteMarker(params, putCb);
        }
    ...
    delete(objectGetInfo, reqUids, callback) {
        ...
        const log = createLogger(reqUids);
        if (deleteVersion && !dataStoreVersionId) {
            // nothing was stored backend-side for this version (e.g. a
            // GCP delete marker): deleting by key would remove live data
    ...
    _copySourceVersionSuffix(sourceVersionId) {
        return sourceVersionId ? `?versionId=${encodeURIComponent(sourceVersionId)}` : '';
    }
    ...
    copyObject(
        ...
        const awsParams = {
            Bucket: this._awsBucketName,
            Key: destAwsKey,
            CopySource: `${sourceAwsBucketName}/${sourceKey}${copySourceVersion}`,
    ...
    uploadPartCopy(request, awsSourceKey, sourceVersionId, sourceLocationConstraintName, config, log, callback) {
        const destBucketName = request.bucketName;
        const destObjectKey = request.objectKey;
        const destAwsKey = this._createAwsKey(destBucketName, destObjectKey, this._bucketMatch);
        const sourceAwsBucketName = config.getAwsBucketName(sourceLocationConstraintName);
        ...
        const params = {
            Bucket: this._awsBucketName,
            CopySource: `${sourceAwsBucketName}/${awsSourceKey}${copySourceVersion}`,
