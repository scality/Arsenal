...
class AwsClient {
    constructor(config) {
        ...
        this._disableContentSha256 = !!config.disableContentSha256;
        
        this._client = this._buildS3Client(this._s3Params);
        
        this._logger = new werelogs.Logger('AwsClient');
    }

    _buildS3Client(params) {
        const clientConfig = { ...params };
        const client = new S3Client(clientConfig);
        this._installContentLengthMiddleware(client);

        if (this._disableContentSha256) {
             ...
             'flexibleChecksumsResponseMiddleware']
                .forEach(name => {
                    try {
                        ...
                        /* no-op: middleware may already be absent */
                    }
                });
        }
    ...
    _installContentLengthMiddleware(client) {
        console.log('[AwsClient] Adding middleware to client');
        client.middlewareStack.add(
            (next, context) => async args => {
                console.log('[Middleware] Running, command:', context.commandName);
                console.log('[Middleware] args.request type:', args.request?.constructor?.name);
                console.log('[Middleware] Has headers?:', !!args.request?.headers);

                const contentLength = args?.input?.ContentLength;
                console.log('[Middleware] ContentLength from input:', contentLength);

                const requestHeaders = args.request?.headers;
                if (contentLength != null && requestHeaders) {
                    const hasChunkedEncoding = Object.keys(requestHeaders)
                        .some(headerName => headerName.toLowerCase() === 'content-encoding'
                            && String(requestHeaders[headerName]).toLowerCase() === 'aws-chunked');
                    const hasContentLength = Object.keys(requestHeaders)
                        .some(headerName => headerName.toLowerCase() === 'content-length');

                    if (hasChunkedEncoding) {
                        console.log('[Middleware] Skipping header update for aws-chunked request');
                    } else if (hasContentLength) {
                        console.log('[Middleware] Content-Length already set, leaving as-is');
                    } else {
                        const lengthStr = String(contentLength);
                        requestHeaders['Content-Length'] = lengthStr;
                        requestHeaders['content-length'] = lengthStr;
                        console.log('[Middleware] Set headers to:', lengthStr);
                    }
                }
            ...
            {
                step: 'build',
                name: 'forceContentLengthMiddleware',
                priority: 'low',
            }
        );
    }
    ...
    setup(cb) {
        ...
        const applyRegion = region => {
            ...
            try {
                this._client = this._buildS3Client(newConfig);
            } catch (err) {
                this._logger.error('unexpected error during setup', {
                    error: err,
                    method: 'AwsClient.setup',
    ...
    uploadPart(request, streamingV4Params, stream, size, key, uploadId, partNumber, bucketName, log, callback) {
        console.log('Uploading part number:', partNumber, 'for uploadId:', uploadId);
        ...
        if (request) {
            ...
            console.log('Prepared part stream piped to MD5Sum for hashing.', hashedStream, partStream);
        }   
        const awsBucket = this._awsBucketName;
        const awsKey = this._createAwsKey(bucketName, key, this._bucketMatch);
        ...
        console.log('[Arsenal uploadPart] Calling UploadPartCommand with params:', params);
        console.log('[Arsenal uploadPart] Client endpoint:', size);
