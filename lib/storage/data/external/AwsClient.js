...
class AwsClient {
    constructor(config) {
        ...
        this._installContentLengthMiddleware(this._client);
    ...
    setup(cb) {
        ...
        const applyRegion = region => {
            ...
            try {
                const regionalClient = new S3Client(newConfig);
                this._installContentLengthMiddleware(regionalClient);
                this._client = regionalClient;
            } catch (err) {
                this._logger.error('unexpected error during setup', {
                    error: err,
                    method: 'AwsClient.setup',
