   ...
   setup(cb) {
        ...
        this._client.send(new GetBucketLocationCommand({ Bucket: this._awsBucketName }))
            ...
            if (region && isAWS) {
                ...
                this._originalEndpoint = endpoint; // Update stored original endpoint
                ...
                // For non-AWS backends, preserve the original endpoint explicitly
                // Always use the stored original endpoint string to avoid SDK v3 provider conversion
                const endpoint = this._originalEndpoint || 
                    (typeof this._s3Params.endpoint === 'string' ? this._s3Params.endpoint : 'http://127.0.0.1:8001');
                const clientParams = {
                    ...this._s3Params,
                    endpoint, // Explicitly set as string, not a provider function
                    region
                };
                // Update _s3Params to keep it in sync
                this._s3Params.endpoint = endpoint;
                this._s3Params.region = region;
                console.log('[AwsClient.setup] Recreating client for non-AWS backend:', {
            ...
            console.error('[AwsClient.setup] Error during setup:', {
                ...
                code: err.code,
                errno: err.errno,
                isAuthorizationHeaderMalformed: err.name === 'AuthorizationHeaderMalformed',
            });
            const isAWS = this._originalEndpoint?.endsWith('amazonaws.com') || 
                         (typeof this._s3Params.endpoint === 'string' && this._s3Params.endpoint.endsWith('amazonaws.com'));
            // For non-AWS backends, if GetBucketLocationCommand fails (e.g., ECONNREFUSED),
            // we should still recreate the client with the correct endpoint and proceed
            if (!isAWS) {
                console.log('[AwsClient.setup] Non-AWS backend, recreating client with endpoint even after error');
                // Always use the original endpoint string to avoid SDK v3 provider conversion issues
                const endpointString = this._originalEndpoint || 
                    (typeof this._s3Params.endpoint === 'string' ? this._s3Params.endpoint : 'http://127.0.0.1:8001');
                const clientParams = {
                    ...this._s3Params,
                    endpoint: endpointString, // Explicitly set as string, not a provider function
                    region: this._s3Params.region || 'us-east-1'
                };
                // Update _s3Params to keep it in sync
                this._s3Params.endpoint = endpointString;
                this._s3Params.region = clientParams.region;
                console.log('[AwsClient.setup] Recreating S3Client after error:', {
                    endpoint: clientParams.endpoint,
                    region: clientParams.region,
                    forcePathStyle: clientParams.forcePathStyle,
                    endpointType: typeof clientParams.endpoint,
                });
                this._client = new S3Client(clientParams);
                this._addRequestLoggingMiddleware();
                // Verify the endpoint was set correctly
                const actualEndpoint = this._client.config?.endpoint;
                console.log('[AwsClient.setup] New client endpoint after error:', {
                    endpointType: typeof actualEndpoint,
                    endpointValue: typeof actualEndpoint === 'function' ? 'FUNCTION (BAD!)' : actualEndpoint,
                    originalEndpoint: this._originalEndpoint,
                });
                // For non-AWS backends, proceed even if GetBucketLocationCommand failed
                // (the backend might not be running yet, but we've configured the client correctly)
                return cb();
            }
            console.error('Error during AwsClient setup:', err);
    ...
    put(stream, size, keyContext, reqUids, callback) {
        ...
        try {
            ...
            console.error('[AwsClient.put] About to create PutObjectCommand with:', {
            ...
            // AWS SDK v3 has automatic checksum calculation that conflicts with our MD5Sum stream
            // Disable SDK's checksum calculation since we're already calculating MD5
            // See: https://github.com/aws/aws-sdk-js-v3/issues/7048
            const command = new PutObjectCommand(uploadParams);
            const requestOptions = {
                requestChecksumCalculation: 'WHEN_REQUIRED',
            };
            
            return this._client.send(command, requestOptions) .then(res => {
                    console.error('[AwsClient.put] Got response from backend!', {
                        ETag: res.ETag,
                ...
                .catch(err => {
                    console.error('[AwsClient.put] Request failed:', {
                        name: err.name,
                        message: err.message,
                        code: err.code,
