    ...
    put(stream, size, keyContext, reqUids, callback) {
        ...
        try {
            ...
            const putCb = (err, data) => {
                ...
                consolele.log('Initial VersionId from data:', data.$metadata?.httpHeaders, versionId);
            ...
            console.error('[AwsClient.put] About to upload:', {
                hasBody: !!uploadParams.Body,
                ContentLength: uploadParams.ContentLength,
                Bucket: uploadParams.Bucket,
            });
            // Use Upload utility from @aws-sdk/lib-storage for proper stream handling
            // Upload automatically handles Content-Length header for streams
            // Don't set partSize for small files - Upload will use single-part PutObject automatically
            // Only set partSize if file is large enough (>= 5MB) to avoid validation errors
            const uploadConfig = {
                client: this._client,
                params: uploadParams,
                leavePartsOnError: false,
            };
            // Only set partSize if size >= 5MB (Upload's minimum part size)
            // For smaller files, Upload will automatically use single-part PutObject
            if (size >= 5242880) {
                uploadConfig.partSize = size;
            }
