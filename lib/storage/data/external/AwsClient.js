...
class AwsClient {
    constructor(config) {
        ...
        this._addRequestLoggingMiddleware();
        this._logger = new werelogs.Logger('AwsClient');
    }
   _addRequestLoggingMiddleware() {
        // Add middleware to log actual HTTP request URL and extract VersionId from responses
        // Note: We don't check for duplicates because we recreate the client during setup
        // and need to re-add middleware each time
        
        // Add request logging middleware
        this._client.middlewareStack.add(next => async (args) => {
            if (args.request) {
                const host = args.request.hostname || args.request.host;
                const protocol = args.request.protocol || 'http';
                const port = args.request.port;
                const path = args.request.path || '/';
                const url = `${protocol}//${host}${port ? `:${port}` : ''}${path}`;
                console.error('[AwsClient] Actual HTTP request URL:', url, {
            ...
            name: `logRequestUrl-${this._dataStoreName}-${Date.now()}`,  // Unique name per instance
            priority: 'high' 
        });
        
        // Add response handling middleware for VersionId extraction
        this._client.middlewareStack.add(next => async (args) => {
            const response = await next(args);
            
            // Extract VersionId from response headers
            if (response && response.output) {
                console.error('[AwsClient] Middleware processing response for:', this._dataStoreName);
                
                // Check response.response.headers (raw HTTP headers)
                let headers = null;
                if (response.response && response.response.headers) {
                    headers = response.response.headers;
                    console.error('[AwsClient] Found headers in response.response.headers');
                }
                
                if (headers) {
                    const versionIdHeader = headers['x-amz-version-id'] || 
                                        headers['X-Amz-Version-Id'];
                    
                    console.error('[AwsClient] Checking for VersionId header:', {
                        hasHeader: !!versionIdHeader,
                        versionId: versionIdHeader,
                        currentOutputVersionId: response.output.VersionId,
                    });
                    
                    // Store VersionId in response.output if not already present
                    if (versionIdHeader && !response.output.VersionId) {
                        response.output.VersionId = versionIdHeader;
                        console.error('[AwsClient] ✅ Captured VersionId from middleware headers:', versionIdHeader);
                    } else if (!versionIdHeader) {
                        console.error('[AwsClient] ❌ No x-amz-version-id header found in response');
                    }
                } else {
                    console.error('[AwsClient] ⚠️  No headers found in response');
                }
            }
            ...
            step: 'deserialize',
            name: `extractVersionId-${this._dataStoreName}-${Date.now()}`,  // Unique name per instance
            priority: 'low'
        });
        
        console.error('[AwsClient] Middleware registered for:', this._dataStoreName);
    }
    ...
    put(stream, size, keyContext, reqUids, callback) {
        console.error('=== AwsClient.put CALLED ===', { size, hasStream: !!stream });
        try {
            ...
            const putCb = (err, data) => {
                ...
                // Try to extract VersionId from response headers if not in response object
                let versionId = data.VersionId;
                console.log('Initial VersionId from data:', data.$metadata?.httpHeaders, versionId);
                if (!versionId && data.$metadata?.httpHeaders) {
                    // Check common header names for VersionId
                    const headers = data.$metadata.httpHeaders;
                    versionId = headers['x-amz-version-id'] || 
                            headers['X-Amz-Version-Id'] ||
                            headers['versionid'] ||
                            headers['VersionId'];
                    
                    console.log('Found VersionId in headers:', versionId);
                    if (versionId) {
                        data.VersionId = versionId;
                    }
                }
                
                console.error('[AwsClient.put] Response from backend:', {
                    hasVersionId: !!data.VersionId,
                    VersionId: data.VersionId,
                    ETag: data.ETag,
                    supportsVersioning: this._supportsVersioning,
                    dataStoreName: this._dataStoreName,
                });
                
                if (!data.VersionId && this._supportsVersioning) {
                    console.error('[AwsClient.put] ERROR: Backend supports versioning but no VersionId in response!');
            ...
            if (keyContext.isDeleteMarker) {
                ...
                return this._client.send(new DeleteObjectCommand(params))
                    .then(res => putCb(null, res))
                    .catch(err => putCb(err));
            }
            ...
            console.error('[AwsClient.put] Initial params:', {
                size,
                hasStream: !!stream,
            ...
            if (!stream) {
                console.error('uploadParams - no stream', uploadParams);
                return this._client.send(new PutObjectCommand(uploadParams))
                    .then(res => putCb(null, res))
                    .catch(err => putCb(err));
            }
            
            // Check if stream is readable and not ended
            if (stream.readableEnded) {
                const err = new Error('Stream already ended before upload');
                console.error('[AwsClient put] Stream already ended!');
                return putCb(err);
            }

            // For zero-length uploads, don't set Body
            if (size === 0) {
                console.error('[AwsClient.put] Zero-length upload, not setting Body/stream');
                return this._client.send(new PutObjectCommand(uploadParams))
                    .then(res => putCb(null, res))
                    .catch(err => putCb(err));
            }

            // For small files with transform streams (like MD5Sum), buffer the stream
            // to ensure consistent VersionId responses and avoid ContentLength issues
            if (size < 5 * 1024 * 1024) {
                console.error('[AwsClient.put] Buffering small stream for reliable VersionId');
                ...
                stream.on('end', () => {
                    ...
                    console.error('[AwsClient.put] Stream buffered, length:', buffer.length);
                    
                    uploadParams.Body = buffer;
                    uploadParams.ContentLength = buffer.length;
                    
                    console.error('[AwsClient.put] Sending PutObjectCommand with buffer:', {
                        ContentLength: uploadParams.ContentLength,
                        Bucket: uploadParams.Bucket,
                        Key: uploadParams.Key,
                    });
                    
                    this._client.send(new PutObjectCommand(uploadParams))
                        .then(res => {
                            console.error('[AwsClient.put] PutObjectCommand response:', {
                                VersionId: res.VersionId,
                                ETag: res.ETag,
                                metadata: res.$metadata,
                            });
                            return putCb(null, res);
                        })
                        .catch(err => {
                            console.error('[AwsClient.put] PutObjectCommand failed:', {
                                name: err.name,
                                message: err.message,
                                code: err.code,
                            });
                ...
                stream.on('error', err => {
                    hasError = true;
                    console.error('[AwsClient.put] Stream error during buffering:', err);
                    return putCb(err);
                });
                
                return; // Exit early, callbacks handled by stream events
            }

            // For larger files, use Upload utility with multipart
            uploadParams.Body = stream;
            
            console.error('[AwsClient.put] Using Upload utility for large file:', {
            ...
            // Set appropriate part size for large files
            if (size >= 100 * 1024 * 1024) {
                uploadConfig.partSize = Math.min(25 * 1024 * 1024, Math.ceil(size / 1000));
            }

            const upload = new Upload(uploadConfig);

            return upload.done().then(async res => {
                console.error('[AwsClient.put] Got response from Upload utility:', {
                    ETag: res.ETag,
                    VersionId: res.VersionId,
                ...
                // If VersionId is missing and versioning is enabled, try HeadObject
                if (!res.VersionId && this._supportsVersioning) {
                    console.error('[AwsClient.put] VersionId missing from Upload response, trying HeadObject...');
                    
                    try {
                        const headResult = await this._client.send(new HeadObjectCommand({
                            Bucket: this._awsBucketName,
                            Key: uploadParams.Key,
                        }));
                        
                        console.error('[AwsClient.put] HeadObject response:', {
                            VersionId: headResult.VersionId,
                            ETag: headResult.ETag,
                        });
                        
                        if (headResult.VersionId) {
                            res.VersionId = headResult.VersionId;
                            console.error('[AwsClient.put] Retrieved VersionId from HeadObject:', headResult.VersionId);
                        }
                    } catch (headErr) {
                        console.error('[AwsClient.put] HeadObject failed:', headErr);
                        // Continue without VersionId - let the existing error handling deal with it
                    }
                }
            ...
            .catch(err => {
                console.error('[AwsClient.put] Upload utility failed:', {
                    name: err.name,
                    message: err.message,
                    code: err.code,
