...
class AwsClient {
    constructor(config) {
        ...
        console.log('=== AwsClient Setup Started ===');
        console.log('Initial S3 Params:', {
            ...this._s3Params,
            // Redact sensitive information
            accessKeyId: this._s3Params.accessKeyId ? '[REDACTED]' : undefined,
            secretAccessKey: this._s3Params.secretAccessKey ? '[REDACTED]' : undefined,
        });
        console.log('Bucket Name:', this._awsBucketName);
        console.log('Bucket Match:', this._bucketMatch);
        console.log('Data Store Name:', this._dataStoreName);
        console.log('Supports Versioning:', this._supportsVersioning);
    
        // Log current client configuration before getBucketLocation
        console.log('\n=== Before getBucketLocation ===');
        console.log('Current Region:', this._client.config.region);
        console.log('Current Endpoint:', this._client.endpoint ? this._client.endpoint.href : 'default');
        console.log('Client Config Endpoint:', this._client.config.endpoint);
        ...
        // Log after region update
        console.log('\n=== After updating to us-east-1 ===');
        console.log('Updated Region:', this._client.config.region);
        console.log('Updated Endpoint:', this._client.endpoint ? this._client.endpoint.href : 'default');
    
        // this request implicitly updates the endpoint for the location
        // the following code explcitly sets it to avoid surprises
        const getBucketLocationParams = { Bucket: this._awsBucketName };
        
        console.log('\n=== Calling getBucketLocation ===');
        console.log('getBucketLocation Params:', getBucketLocationParams);
        console.log('Client Endpoint before call:', this._client.endpoint ? this._client.endpoint.href : 'default');
        console.log('Client Region before call:', this._client.config.region);
    
        this._client.getBucketLocation(getBucketLocationParams, (err, res) => {
            ...
            if (err) {
                ...
                console.log('Success response:', {
                    LocationConstraint: res.LocationConstraint,
                });
            }
    
            if (err && err.code !== 'AuthorizationHeaderMalformed') {
                console.log('\n=== Setup Failed ===');
                console.log('Setup failed with error:', err);
            ...
            if (err && err.code === 'AuthorizationHeaderMalformed') {
                ...
                region = err.region;
                console.log('\n=== Using region from AuthorizationHeaderMalformed error ===');
                console.log('Error region:', err.region);
            } else if (res) {
                region = res.LocationConstraint;
                console.log('\n=== Using region from getBucketLocation response ===');
                console.log('Response region:', res.LocationConstraint);
            }
    
            // Log region determination
            console.log('\n=== Region Determined ===');
            console.log('Determined region:', region);
            console.log('Source:', err ? 'error' : 'response');
    
            // Update client config with determined region
            this._client.config.update({ region });
            
            console.log('\n=== Client config updated with determined region ===');
            console.log('Updated region:', this._client.config.region);
            console.log('Endpoint before regional update:', this._client.endpoint ? this._client.endpoint.href : 'default');
    
            const isAWS = this._s3Params.endpoint.endsWith('amazonaws.com');
            console.log('\n=== Checking if endpoint is AWS ===');
            console.log('Original endpoint:', this._s3Params.endpoint);
            console.log('Is AWS endpoint:', isAWS);
            console.log('Region for regional endpoint:', region);
    
            if (region && isAWS) {
                const endpoint = `s3.${region}.amazonaws.com`;
                console.log('\n=== Setting regional endpoint ===');
                console.log('New regional endpoint:', endpoint);
                console.log('Old endpoint:', this._client.endpoint ? this._client.endpoint.href : 'default');
                ...
                console.log('Final endpoint after update:', this._client.endpoint.href);
            } else {
                console.log('\n=== Not setting regional endpoint ===');
                console.log('Reason:', !region ? 'no region determined' : 'not AWS endpoint');
            }
    
            // Log final configuration
            console.log('\n=== AwsClient Setup Completed ===');
            console.log('Final config:', {
                region: this._client.config.region,
                endpoint: this._client.endpoint ? this._client.endpoint.href : 'default',
                bucketName: this._awsBucketName,
            });
            console.log('==================================\n');
    ...
    put(stream, size, keyContext, reqUids, callback) {
        ...
        console.log('\n=== AwsClient PUT Operation ===');
        console.log('AWS Key:', awsKey);
        console.log('Bucket Name:', this._awsBucketName);
        console.log('Client Region:', this._client.config.region);
        console.log('Client Endpoint:', this._client.endpoint ? this._client.endpoint.href : 'default');
        console.log('Size:', size);
        console.log('Meta Headers:', metaHeaders);
        ...
        console.log('uploadParams2', JSON.stringify(uploadParams, null, 2));
    ...
    get(objectGetInfo, range, reqUids, callback) {
        console.log('\n=== AwsClient GET Operation ===');
        console.log('Object Get Info:', objectGetInfo);
        console.log('Range:', range);
        console.log('AWS Bucket:', this._awsBucketName);
        console.log('Client Region:', this._client.config.region);
        console.log('Client Endpoint:', this._client.endpoint ? this._client.endpoint.href : 'default');
        const log = createLogger(reqUids);
        const { key, dataStoreVersionId } = objectGetInfo;
    ...
    uploadPart(request, streamingV4Params, stream, size, key, uploadId,
        ...
        console.log('\n=== AwsClient uploadPart Operation ===');
        console.log('AWS request sent',{request})
        let hashedStream = stream;
        console.log('stream type:', typeof stream);
        console.log('Hashed Stream before prepareStream:', hashedStream);
        ...
        if (request) {
            ...
            console.log('Hashed Stream after prepareStream:', hashedStream);
        ...
        console.log('uploadPart params:', params);
