...
class AwsClient {
    ...
    _addRequestLoggingMiddleware() {
        // Add middleware to log actual HTTP request URL
        // This helps debug endpoint configuration issues
        // Check if middleware already exists to avoid duplicates
        const middlewareIds = this._client.middlewareStack.identify();
        if (middlewareIds.includes('logRequestUrl')) {
            return; // Middleware already added
        }
        this._client.middlewareStack.add(next => async (args) => {
            ...
            // Capture response headers for VersionId extraction (needed even when versioning not configured)
            // Headers may be in result.response.headers or result.output.$metadata.httpHeaders
            if (response) {
                let headers = null;
                console.log('[AwsClient] Response received in middleware:', JSON.stringify(response));
                if (response.response && response.response.headers) {
                    headers = response.response.headers;
                } else if (response.output && response.output.$metadata && response.output.$metadata.httpHeaders) {
                    headers = response.output.$metadata.httpHeaders;
                }
                if (headers) {
                    ...
                    if (versionIdHeader && response.output && !response.output.VersionId) {
            ...
            return response;
        }, { step: 'finalizeRequest', name: 'logRequestUrl', priority: 'high' });
    }
        ...
        const attemptSetup = () => {
            ...
            tempClient.send(new GetBucketLocationCommand({ Bucket: this._awsBucketName }))
                .then(res => {
                    ...
                    // Final success. Stop retries and return.
                    return cb();
                })
                .catch(err => {
