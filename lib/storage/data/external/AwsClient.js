const { S3Client,
    ...
    NotFound,
    NoSuchVersion,
    AccessDenied } = require('@aws-sdk/client-s3');
...
class AwsClient {
    ...
    delete(objectGetInfo, reqUids, callback) {
        ...
        return this._client.send(new DeleteObjectCommand(params)).then(() => callback()).catch(err => {
            ...
            if (err instanceof NoSuchVersion || err instanceof NoSuchKey) {
    ...
    listParts(key, uploadId, bucketName, partNumberMarker, maxParts, log,
        ...
        return this._client.send(new ListPartsCommand(params)).catch(err => {
