...
function gcpSigningMiddleware(config) {
    return (next, context) => async (args) => {
        ...
        const fakeRequest = {
            ...
            virtualHostedBucket: undefined, // Set if needed
            bucketName: config.bucketName,
            query: request.query || {},
        };
        // If using virtual hosted style, set virtualHostedBucket
        if (request.hostname && config.bucketName && request.hostname.startsWith(config.bucketName + '.')) {
            fakeRequest.virtualHostedBucket = config.bucketName;
            fakeRequest.gotBucketNameFromHost = true;
        }
        const data = {};
        const logger = { trace: () => {} };
