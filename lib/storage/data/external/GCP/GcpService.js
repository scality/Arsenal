...
class GcpClient extends S3Client {
    ...
    completeMultipartUpload(params, callback) {
        ...
        return async.waterfall([
            ...
            (numParts, next) => {
                ...
                console.log('[GcpClient] completeMultipartUpload: composed final object');
            },
            (result, next) => {
                mpuHelper.generateMpuResult(result, partList, next);
                console.log('[GcpClient] completeMultipartUpload: generated MPU result');
            },
            (result, aggregateETag, next) => {
                mpuHelper.copyToMain(result, aggregateETag, params, next);
                console.log('[GcpClient] completeMultipartUpload: copied to main bucket', result, aggregateETag);
            },
            (mpuResult, next) => {
                const delParams = {
                    Bucket: params.Bucket,
                ...
                console.log('[GcpClient] completeMultipartUpload: cleaning up parts with params:', delParams);
                mpuHelper.removeParts(delParams, err => {
                    console.log('[GcpClient] completeMultipartUpload cleanup err:', err);
                    next(err, mpuResult);
                });
            },
        ], (err, result) => {
            console.log('[GcpClient] completeMultipartUpload err:', err);
            console.log('[GcpClient] completeMultipartUpload result:', result);
            callback(err, result);
        });
    }
    ...
    deleteObjectTagging(params, callback) {
        ...
        return async.waterfall([
            next => {
                console.log('[deleteObjectTagging] Step 1: headObject');
                this.headObject({
                    Bucket: params.Bucket,
                    Key: params.Key,
                    VersionId: params.VersionId,
                }, (err, res) => {
                    console.log('[deleteObjectTagging] headObject result - err:', err, 'Metadata:', res?.Metadata);
                    next(err, res);
                });
            },
            (resObj, next) => {
                console.log('[deleteObjectTagging] Step 2: stripTags and copyObject');
                console.log('[deleteObjectTagging] Original metadata:', resObj.Metadata);
                const completeMD = stripTags(resObj.Metadata);
                console.log('[deleteObjectTagging] After stripTags:', completeMD);
                ...
                if (Object.keys(completeMD).length === 0) {
                    ...
                    console.log('[deleteObjectTagging] Added dummy metadata for REPLACE');
                }
                const copyParams = {
                    Bucket: params.Bucket,
                    Key: params.Key,
                ...
                console.log('[deleteObjectTagging] copyObject params.Metadata:', copyParams.Metadata);
                this.copyObject(copyParams, (err, res) => {
                    console.log('[deleteObjectTagging] copyObject result - err:', err);
                    next(err, res);
                });
            },
        ], (err, result) => {
            console.log('[deleteObjectTagging] Complete - err:', err);
            callback(err, result);
        });
    }
    ...
    headObject(params, callback) {
        console.log('[headObject] Called for Key:', params.Key);
        ...
        return this.send(command)
            .then(data => {
            console.log('[headObject] Promise resolved, data keys:', Object.keys(data));
            ...
            // Merge SDK response with normalized metadata
        ...
        try {
            ...
            Object.keys(request.headers).forEach(header => {
                ...
                // Convert x-amz-meta-* to x-goog-meta-*
                    ...
                    console.log(`[GCP Signing] Converted metadata header: ${header} → ${gcpMetaKey} = ${request.headers[header]}`);
                    delete request.headers[header];
                }
            });
            
            // Log all metadata headers being sent
            const metadataHeaders = Object.keys(request.headers).filter(h => h.toLowerCase().startsWith('x-goog-meta-'));
            if (metadataHeaders.length > 0) {
                console.log('[GCP Signing] Sending metadata headers:', metadataHeaders);
            }
            
            // Log if REPLACE directive is set
            if (request.headers['x-goog-metadata-directive']) {
                console.log('[GCP Signing] Metadata directive:', request.headers['x-goog-metadata-directive']);
            };
