...
class GcpClient extends S3Client {
    ...
    deleteObject(params, callback) {
        ...
        if (deleteParams.VersionId) {
            console.log('[deleteObject] Removing VersionId for GCP:', deleteParams.VersionId);
            delete deleteParams.VersionId;
        }
        console.log('[deleteObject] Deleting object with params:', deleteParams);
    ...
    composeObject(params, callback) {
        console.log('[composeObject] Called with', params.MultipartUpload?.Parts?.length, 'parts for key:', params.Key);
        ...
        console.log('[composeObject] Parts to compose:', parts.map(p => p.PartName));
        ...
        return this.send(command)
            .then(data => {
                console.log('[composeObject] Success! Created:', params.Key);
                return callback && callback(null, data);
            })
            .catch(err => {
                console.log('[composeObject] Error:', err.name, 'for key:', params.Key);
                return callback?.(err);
            });
    }
        ...
        try {
            ...
            Object.keys(request.headers).forEach(header => {
                ...
                if (convertHeaders[lowerHeader]) {
                    ...
                    console.log(`[GCP Signing] BEFORE SIGN: Converted ${header} to ${convertHeaders[lowerHeader]}`);
