...
class GcpClient extends S3Client {
   ...
   /**
    ...
    completeMultipartUpload(params, callback) {
        console.log('[completeMPU] CALLED with params:', {
            Bucket: params.Bucket,
            Key: params.Key,
            UploadId: params.UploadId,
            PartsCount: params.MultipartUpload?.Parts?.length,
        });
        console.log('[completeMPU] callback type:', typeof callback);
        
        if (!params || !params.MultipartUpload ||
            !params.MultipartUpload.Parts || !params.UploadId ||
            !params.Bucket || !params.Key) {
            console.log('[completeMPU] Missing parameters!');
            const error = errorInstances.InvalidRequest
                .customizeDescription('Missing required parameter');
            return callback(error);
        }
        ...
        console.log('[completeMPU] Starting waterfall...');
        return async.waterfall([
            next => {
                console.log('[completeMPU] Step 1: splitMerge', partList.length, params);
                mpuHelper.splitMerge(params, partList, 'compose', (...args) => {
                    console.log('[completeMPU] Step 1 done, args:', args.length);
                    next(...args);
                });
            },
            (numParts, next) => {
                console.log('[completeMPU] Step 2: composeFinal, numParts:', numParts);
                mpuHelper.composeFinal(numParts, params, (...args) => {
                    console.log('[completeMPU] Step 2 done');
                    next(...args);
                });
            },
            (result, next) => {
                console.log('[completeMPU] Step 3: generateMpuResult');
                mpuHelper.generateMpuResult(result, partList, (...args) => {
                    console.log('[completeMPU] Step 3 done');
                    next(...args);
                });
            },
            (result, aggregateETag, next) => {
                console.log('[completeMPU] Step 4: copyToMain');
                mpuHelper.copyToMain(result, aggregateETag, params, (...args) => {
                    console.log('[completeMPU] Step 4 done');
                    next(...args);
                });
            },
            (mpuResult, next) => {
                console.log('[completeMPU] Step 5: removeParts');
                const delParams = {
                    Bucket: params.Bucket,
                    MPU: params.MPU,
                ...
                mpuHelper.removeParts(delParams, err => {
                    console.log('[completeMPU] Step 5 done, err:', err);
                    next(err, mpuResult);
                });
            },
        ], (err, result) => {
            console.log('[completeMPU] Waterfall complete - err:', err, 'result:', result);
            callback(err, result);
        });
    }
...
function gcpSigningMiddleware(config) {
    return (next, context) => async (args) => {
        try {
            ...
            const keepHeaders = ['x-amz-copy-source', 'x-amz-metadata-directive', 'x-amz-copy-source-if-match',
                'x-amz-copy-source-if-none-match', 'x-amz-copy-source-if-modified-since',
                'x-amz-copy-source-if-unmodified-since'];
            Object.keys(request.headers).forEach(header => {
                const lowerHeader = header.toLowerCase();
                if (lowerHeader.startsWith('x-amz-')) {
                    if (keepHeaders.includes(lowerHeader)) {
                        console.log('[GCP Signing] KEEPING header:', header);
                    } else {
                    ...
                    // Remove any other AWS SDK headers
                    removedHeaders.push(header);
                    delete request.headers[header];
                }
            });
            console.log('[GCP Signing] Removed x-amz headers:', removedHeaders);
            console.log('[GCP Signing] ALL headers after removal:', Object.keys(request.headers));
            return next(args);
        }
        catch (err) {
