...
function gcpSigningMiddleware(config) {
    return (next, context) => async (args) => {
        try {
            ...
            // Extract bucket name from request path (path-style) or hostname (virtual-hosted-style)
            let bucketName = config.bucketName;
            let virtualHostedBucket = undefined;
            
            // Check for virtual-hosted-style (bucket in hostname)
            if (request.hostname && config.bucketName && request.hostname.startsWith(config.bucketName + '.')) {
                virtualHostedBucket = config.bucketName;
                bucketName = config.bucketName;
            } else {
                // Extract from path for path-style requests
                const bucketMatch = request.path.match(/^\/([^\/\?]+)/);
                if (bucketMatch) {
