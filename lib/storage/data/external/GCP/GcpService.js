...
class GcpClient extends S3Client {
    ...
    completeMultipartUpload(params, callback) {
        ...
        return async.waterfall([
            ...
            (mpuResult, next) => {
                ...
                mpuHelper.removeParts(delParams, err => next(err, mpuResult));
            },
    ...
    deleteObject(params, callback) {
        // GCP uses numeric generation IDs, not ETags
        // Filter out VersionId if it looks like an ETag (quoted MD5)
        const deleteParams = { ...params };
        if (deleteParams.VersionId && deleteParams.VersionId.match(/^"[a-f0-9]{32}"$/i)) {
            console.log('[deleteObject] Removing ETag-like VersionId for GCP:', deleteParams.VersionId);
