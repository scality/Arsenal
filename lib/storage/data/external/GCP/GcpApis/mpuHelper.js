...
class MpuHelper {
    ...
    removeParts(params, callback) {
        const _getObjectVersions = callback => {
            ...
            return async.whilst(() => isTruncated, next => {
                ...
                return this.service.listVersions(listParams, (err, res) => {
                    console.log('[GcpClient] removeParts: listed versions with params:', listParams, 'response:', res, 'err:', err);
                    if (err) {
                ...
                this.service.deleteObject(delParams, err => {
                    if (err) {
                        if (err.name === 'NoSuchKey' || err.code === 'NoSuchKey') {
                            logHelper(logger, 'warn', 'object already deleted (ignoring)', { Key: obj.Key });
                            return next();
                        }
                        logHelper(logger, 'error',
        ...
        console.log('[GcpClient] removeParts: starting removal of parts with prefix:', params.Prefix);
