...
class MpuHelper {
    ...
    splitMerge(params, partList, level, callback) {
        console.log('[splitMerge] Called with partList length:', partList.length, 'level:', level);
        // create composition of slices from the partList array
        const slices = eachSlice.call(partList, 32);
        console.log('[splitMerge] Created', slices.length, 'slices');
        console.log('[splitMerge] maxConcurrent:', this.service._maxConcurrent);
        // create composition of slices from the partList array
