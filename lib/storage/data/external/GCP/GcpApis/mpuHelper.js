...
const Backoff = require('../../../../../utils/Backoff').default;
const { eachSlice, createMpuKey, createMpuList, logger } = require('../GcpUtils');
const { logHelper } = require('../../utils');
const { createAggregateETag } = require('../../../../../s3middleware/processMpuParts');
...
class MpuHelper {
    ...
    _retry(fnName, params, callback) {
        ...
        const handleFunc = (fnName, params, retry, callback) => {
            ...
            return setTimeout(
                (params, cb) => this.service[fnName](params, cb),
                timeout,
                params,
                (err, res) => {
                    if (err) {
                        if (err.statusCode === 429 || err.code === 429) {
                            if (fnName === 'composeObject') {
                                logger.trace('composeObject: slow down request', { retryCount: retry, timeout });
                            } else if (fnName === 'copyObject') {
                                logger.trace('copyObject: slow down request', { retryCount: retry, timeout });
                            }
                            return handleFunc(fnName, params, retry + 1, callback);
                        }
    ...
    splitMerge(params, partList, level, callback) {
        ...
        return async.mapLimit(
            eachSlice.call(partList, 32),
            this.service._maxConcurrent,
            (infoParts, cb) => {
                const mpuPartList = infoParts.Parts.map(item => ({ PartName: item.PartName }));
                const partNumber = infoParts.PartNumber;
                const tmpKey = createMpuKey(params.Key, params.UploadId, partNumber, level);
                ...
                if (mpuPartList.length < 2) {
                    ...
                    return this.service.copyObject(copyParams, (err, res) => {
                        if (err) {
                            logHelper(logger, 'error', 'error in splitMerge - copyObject', err);
                            return cb(err);
                        }
                        mergedObject.VersionId = res.CopyObjectResult.VersionId;
                        mergedObject.ETag = res.CopyObjectResult.ETag;
    ...
    removeParts(params, callback) {
        logger.trace('removeParts: starting cleanup', { prefix: params.Prefix });
        let totalDeleted = 0;
        ...
        return async.whilst(
            ...
            next => {
                ...
                const command = new ListObjectVersionsCommand(listParams);
                this.service.send(command).then(res => {
                    nextKeyMarker = res.NextKeyMarker;
                    nextVersionIdMarker = res.NextVersionIdMarker;
                    isTruncated = res.IsTruncated;

                    const versions = res.Versions || [];
                    logger.trace('removeParts: got page', {
                            ...
                            this.service.deleteObject(delParams, err => {
                                if (err) {
                                    if (err.name === 'NoSuchKey') {
                                        logHelper(logger, 'warn', 'removeParts: object already deleted (ignoring)', {
                                            Key: obj.Key,
                                        });
                                        return done();
                                    }
                                    logHelper(logger, 'error', 'removeParts: deleteObject failed', err);
                                    return done(err);
                                }
                                totalDeleted++;
                                return done();
            ...
            err => {
                if (err) {
                    logger.error('removeParts: failed', { error: err, totalDeleted });
                } else {
                    logger.trace('removeParts: completed', { totalDeleted });
                }
                callback(err);
            },
        );
    ...
    composeFinal(numParts, params, callback) {
        ...
        const partList = parts.map(item => ({ PartName: item.PartName }));
        if (partList.length < 2) {
            logger.trace('fewer than 2 parts, skip to copy phase');
    ...
    generateMpuResult(res, partList, callback) {
        const partETags = partList.map(part => part.ETag.substring(1, part.ETag.length - 1));
    ...
    copyToMain(res, aggregateETag, params, callback) {
        ...
        const copySource = res || createMpuKey(params.Key, params.UploadId, 'final');
            ...
            [
                next => {
                    ...
                    return this.service.headObject(headParams, (err, res) => {
                        if (err) {
                            logHelper(logger, 'error', 'error in createMultipartUpload - headObject', err);
                            return next(err);
                        }
                ...
                (res, next) => {
                    ...
                    this.retryCopy(copyParams, (err, res) => {
                        if (err) {
                            logHelper(logger, 'error', 'error in ' + 'createMultipartUpload - final copyObject', err);
                            return next(err);
                        }
                        const mpuResult = {
                            Bucket: params.Bucket,
                            ...
                            (err, res) => {
                                if (err) {
                                    logHelper(
                                        logger,
                                        'error',
                                        'error in ' + 'createMultipartUpload - final head object',
                                        err,
                                    );
                                    return next(err);
                                }
