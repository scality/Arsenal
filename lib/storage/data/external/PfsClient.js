...
class PfsClient {
    ...
    put(stream, size, keyContext, reqUids, callback) {
        ...
        if (keyContext.metaHeaders['x-amz-meta-mdonly'] === 'true' || keyContext.isDeleteMarker === true) {
            const b64 = keyContext.metaHeaders['x-amz-meta-md5chksum'];
            let md5 = null;
            if (b64) {
                md5 = Buffer.from(b64, 'base64').toString('hex');
            }
            return callback(null, keyContext.objectKey, '', keyContext.metaHeaders['x-amz-meta-size'], md5);
        }
        logHelper(log, 'error', 'Not implemented', errors.NotImplemented, this._dataStoreName, this.clientType);
