...
/**
 * Describe a proxy URL as a mutable plain object, so the caller can merge cert
 * options into it before handing it to the agent.
 * @param {string} proxyUrl - absolute URL of the outbound proxy
 * @return {object} agent options describing the proxy
 */
function proxyAgentOptions(proxyUrl) {
    const parsed = new URL(proxyUrl);
    const auth = parsed.username ? `${parsed.username}${parsed.password ? `:${parsed.password}` : ''}` : null;
    return {
...
function parseLC(config, vault) {
    ...
    Object.keys(config.locationConstraints).forEach(location => {
        ...
        if (locationObj.type === 'scality') {
            if (locationObj.details.connector.sproxyd) {
                clients[location] = new Sproxy({
                    bootstrap: locationObj.details.connector.sproxyd.bootstrap,
                ...
                clients[location] = new Hyperdrive.hdcontroller.HDProxydClient(locationObj.details.connector.hdclient);
                clients[location].clientType = 'scality';
            }
        }
        if (locationObj.type === 'aws_s3' || locationObj.type === 'gcp') {
            let connectionAgent;
            const endpoint =
                locationObj.type === 'gcp' ? locationObj.details.gcpEndpoint : locationObj.details.awsEndpoint;
            const region = locationObj.details.region || 'us-east-1';
            ...
            // keepalive config
            const httpAgentConfig = config.externalBackends[locationObj.type].httpAgent;
            ...
            if (config.outboundProxy.url && !proxyMatch) {
                const options = proxyAgentOptions(config.outboundProxy.url);
                ...
                connectionAgent = sslEnabled ? new https.Agent(agentOptions) : new http.Agent(agentOptions);
            }
            ...
            const s3Params = {
                ...
                requestHandler: new NodeHttpHandler({
                    requestTimeout: 0,
                    ...(sslEnabled ? { httpsAgent: connectionAgent } : { httpAgent: connectionAgent }),
                }),
            ...
            clients[location] = locationObj.type === 'gcp' ? new GcpClient(clientConfig) : new AwsClient(clientConfig);
        ...
        if (locationObj.type === 'azure') {
            ...
            const proxyParams = backendUtils.proxyCompareUrl(azureStorageEndpoint) ? {} : config.outboundProxy;
            const azureStorageCredentials = config.getAzureStorageCredentials(location);
