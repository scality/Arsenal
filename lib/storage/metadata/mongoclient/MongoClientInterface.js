...
class MongoClientInterface {
    ...
    getObjects(bucketName, objects, log, callback) {
        ...
        const processDoc = (doc, objName, params, versionId, cb) => {
            if (!doc && params && params.versionId) {
        ...
        this.getBucketVFormat(bucketName, log, (err, _vFormat) => {
            ...
            return c.find({
                ...
                async.mapLimit(keysAndObjects, 5, ({ key, objName, params, versionId }, cb) => {
                    const doc = docByKey.get(key);
                    processDoc(doc, objName, params, versionId, cb);
                }, callback);
            }).catch(err => {
                callback(err);
            });
     ...
     * @return {undefined}
     */
    async internalDeleteObject(collection, bucketName, key, filter, log, cb, originOp = 's3:ObjectRemoved:Delete') {
        ...
        try {
            const doc = await collection.findOneAndUpdate(findFilter, {
                $set: {
                    '_id': key,
                    'value.deleted': true,
                },
            ...
            // Now that .deleted flag is set, launch the bulkWrite operation
            ...
            collection.bulkWrite([
                {
                    updateOne: {
                        ...
                        update: {
                            $set: { _id: key, value: objMetadata.getValue() },
                        },
                        upsert: false,
                    },
                }, {
                ...
                log.error('internalDeleteObject: error during bulk write operation',
                    { bucket: bucketName, object: key, error: err.message });
            });

            return cb(null, objMetadata.getValue());
        } catch (err) {
