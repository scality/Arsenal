...
class MongoClientInterface {
        ...
        this.db.listCollections().toArray((err, collInfos) => {
            async.eachLimit(collInfos, 10, (value, next) => {
                ...
                return async.parallel({
                    bucketList: done => this.getBucketAttributes(
                    ...
                    counter: done =>
                        this.getObjectMDStats(bucketName, log, done),
                }, (err, results) => {
                    if (err) {
    ...
    getDataInfo(c, mstVerIds, log, callback) {
        ...
        return async.series({
            master: done => {
                ...
                /* eslint-enable quote-props */
                return this.getDataManaged(c, query, done);
            },
            archived: done => {
    ...
    getObjectMDStats(bucketName, log, callback) {
        const c = this.getCollection(bucketName);
        c.distinct('_id').then(keys => {
            ...
            return async.map(uniqKeys, (key, done) => {
                this.getLatestVersion(c, key, log, (err, mst) => {
                    if (err) {
                        log.debug('unable to retrieve master info');
                        return done(err);
                    }
                ...
                return this.getDataInfo(c, mstVerIds, log, callback);
            });
        }).catch(callback);
    }
}
