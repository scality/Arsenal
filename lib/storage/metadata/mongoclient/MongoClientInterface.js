...
class MongoClientInterface {
    ...
    deleteObjectVerMaster(c, bucketName, objName, params, log, cb) {
        ...
        async.waterfall([
            next => c.find({
                // We try to get both the master object and the latest version
                // of the object as there are cases where only one of them is available.
                // When the last version is a delete marker only the version is available.
                // In versioning suspended buckets, we can have masters that do not have versions,
                // the oher case is also possible where an object has both the master and versions.
                // That is why we use the same deletion logic for both versioned and versioning suspended
                // objects.
                $or: [{
                    _id: masterKey,
                }, {
                ...
                if (err) {
                    log.error('deleteObjectVerMaster: error getting object',
                        { bucket: bucketName, object: objName, error: err.message });
                    return next(errors.InternalError);
                }
                if (!doc) {
                    log.error('deleteObjectVerMaster: unable to find target object',
                ...
                // updating versionId
                objMetadata.setVersionId(_vid);
                return next(null, objMetadata.getValue());
            }),
            // Updating/creating master object
            (mst, next) => c.updateOne(
                { _id: masterKey },
                {
                ...
                err => next(err, mst)),
            // deleting version
            (mst, next) => this.internalDeleteObject(c, bucketName, versionKey, {}, log,
                err => {
    ...
    internalDeleteObject(collection, bucketName, key, filter, log, cb) {
        ...
        const findUpdateFilter = Object.assign({
            _id: key,
        }, filter);
        ...
        async.waterfall([
            next => collection.findOne(findUpdateFilter, {}, (err, doc) => {
                if (err) {
                ...
                const objMetadata = new ObjectMD(doc.value);
                // adding delete flag to object metadata
