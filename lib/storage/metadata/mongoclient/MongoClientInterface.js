...
class MongoClientInterface {
    ...
    /**
     * Creates the PHD master if not already existing with
     * the specified version object's metadata
     * @param {Collection} c mongo collection
     * @param {string} bucketName bucket name
     * @param {string} masterKey master object's key
     * @param {string} versionKey version object's key
     * @param {string} phdVersionId phd versionId
     * @param {Object} log logger
     * @param {Function} cb callback
     * @return {undefined}
     */
    createPHDMasterWithVersionMD(c, bucketName, masterKey, versionKey, phdVersionId, log, cb) {
        async.waterfall([
            next => c.findOne({
                _id: versionKey,
            }, {}, next),
            (doc, next) => {
                if (doc) {
                    ...
                    // adding PHD flag to object metadata
                    objMetadata.setIsPHD(true);
                    // updating versionId
                    objMetadata.setVersionId(phdVersionId);
                    return next(null, objMetadata.getValue());
                }
                return next(errors.NoSuchKey);
            },
            // this creates master only when it's non existent
            // if master got created in the meantime it means the
            // object got updated
            (md, next) => c.insertOne({
                _id: masterKey,
                value: md,
            ...
            // error code 11000 means that an object with same _id already exists
            // we ignore those cause they mean that the object got updated
            if (err && err.code !== 11000) {
                if (err.is.NoSuchKey) {
                    log.error('createPHDMasterWithVersionMD: error finding version key',
                        { error: err.message, bucket: bucketName, key: versionKey });
                    return cb(errors.NoSuchKey);
                }
                log.error('createPHDMasterWithVersionMD: error creating master object',
                    { error: err.message, bucket: bucketName, key: masterKey });
                return cb(errors.InternalError);
            }
            return cb(null);
        });
    ...
    deleteObjectVerMaster(c, bucketName, objName, params, log, cb) {
        ...
        async.waterfall([
            ...
            (resp, next) => {
                // Create master key if non existent (tagged with PHD)
                if (resp.modifiedCount === 0) {
                    return this.createPHDMasterWithVersionMD(c, bucketName, masterKey, versionKey, _vid, log, next);
                }
                return next(null);
            },
            // delete version
    ...
    deleteObjectVerNotMaster(c, bucketName, objName, params, log, cb) {
        const versionKey = formatVersionKey(objName, params.versionId, params.vFormat);
        this.internalDeleteObject(c, bucketName, versionKey, {}, log, (err, res) => {
            if (err) {
                if (err.is.NoSuchKey) {
                    log.error(
            ...
            if (res.ok !== 1) {
                log.error(
                    'deleteObjectVerNotMaster: failed to delete object',
                    { bucket: bucketName, object: objName, versionId: params.versionId });
                return cb(errors.InternalError);
            }
            return cb(null);
        });
    ...
    deleteObjectNoVer(c, bucketName, objName, params, log, cb) {
        const masterKey = formatMasterKey(objName, params.vFormat);
        this.internalDeleteObject(c, bucketName, masterKey, {}, log, (err, res) => {
            if (err) {
            ...
            if (res.ok !== 1) {
                log.error(
                    'deleteObjectNoVer: failed to delete object',
                    { bucket: bucketName, object: objName });
                return cb(errors.InternalError);
            }
            return cb(null);
        });
