...
class MongoClientInterface {
    ...
    consolidateData(store, results) {
        ...
        if (results.dataManaged
            ...
            store.dataManaged.total.curr += results.dataManaged.total.curr;
            store.dataManaged.total.prev += results.dataManaged.total.prev;
            Object.keys(locations).forEach(site => {
                if (!store.dataManaged.byLocation[site]) {
                    store.dataManaged.byLocation[site] = { ...locations[site] };
                } else {
        ...
        if (results.dataMetrics
            ...
            Object.keys(results.dataMetrics).forEach(metricLevel => {
                Object.keys(results.dataMetrics[metricLevel]).forEach(resourceName => {
                    if (store.dataMetrics[metricLevel][resourceName]) {
                        Object.keys(store.dataMetrics[metricLevel][resourceName]).forEach(subMetric1 => {
                            Object.keys(store.dataMetrics[metricLevel][resourceName][subMetric1]).forEach(
                                subMetric2 => {
                                    store.dataMetrics[metricLevel][resourceName][subMetric1][subMetric2]
                                    += results.dataMetrics[metricLevel][resourceName][subMetric1][subMetric2];
                                });
                        });
                    } else {
                        store.dataMetrics[metricLevel][resourceName] = results.dataMetrics[metricLevel][resourceName];
                    }
                });
            });
        } else {
            store.dataMetrics = results.dataMetrics;
        }
        /* eslint-enable */
    }
    ...
    scanItemCount(log, cb) {
        ...
        const consolidateData = results =>
            this.consolidateData(store, results);
    ...
    _handleResults(res, isVersioned) {
        ...
        return {
            versions: totalNonCurrentCount > 0 ? totalNonCurrentCount : 0,
