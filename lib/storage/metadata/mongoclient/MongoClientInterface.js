...
class MongoClientInterface {
    ...
    internalDeleteObject(collection, bucketName, key, filter, log, cb, originOp = 's3:ObjectRemoved:Delete') {
        ...
        async.waterfall([
            ...
            ], { ordered: true }).then(next).catch(next),
        ], (err, res) => {
            if (err) {
                if (err.is.NoSuchKey) {
                    return cb(err);
    ...
    putBucketIndexes(bucketName, indexSpecs, log, cb) {
        const c = this.getCollection(bucketName);
        c.createIndexes(indexSpecs, err => {
            if (err) {
                log.error(
    ...
    deleteBucketIndexes(bucketName, indexSpecs, log, cb) {
        ...
        async.each(indexSpecs,
            (spec, next) => c.dropIndex(spec.name, next),
            err => {
                if (err) {
                    log.error(
    ...
    getBucketIndexes(bucketName, log, cb) {
        ...
        c.listIndexes().toArray((err, res) => {
            ...
            return cb(null, MongoUtils.indexFormatMongoArrayToObject(res));
        });
    }
}
