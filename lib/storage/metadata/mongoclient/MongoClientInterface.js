...
const NUM_CONCURRENT_BUCKETS_CNT = 10;
...
class MongoClientInterface {
    ...
    scanItemCount(log, cb) {
        ...
        this._getBucketInfos(log, (err, res) => {
            ...
            let concurrentBuckets = NUM_CONCURRENT_BUCKETS_CNT;
            if (process.env.NUM_CONCURRENT_BUCKETS_CNT &&
                !Number.isNaN(process.env.NUM_CONCURRENT_BUCKETS_CNT)) {
                concurrentBuckets = Number
                    .parseInt(process.env.NUM_CONCURRENT_BUCKETS_CNT, 10);
            }

            return async.eachLimit(bucketInfos, concurrentBuckets,
                (bucketInfo, done) => {
                    async.waterfall([
    ...
    _processEntryData(entry, isTransient) {
        const results = {};
        if (!isTransient || entry.replicationInfo.status !== 'COMPLETED') {
            results[entry.value.dataStoreName] = entry.value['content-length'];
        }
        entry.replicationInfo.backends.forEach(rep => {
     ...
     */
    _procesEntryStalled(entry, cmpDate) {
        ...
        cursor.forEach(
            res => {
                ...
                if (res._id.indexOf('\0') !== -1) {
                    ...
                    if (this._procesEntryStalled(res, cmpDate)) {
                        stalledCount++;
                    }
