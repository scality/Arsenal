...
class MongoClientInterface {
    ...
    getObjects(bucketName, objects, log, callback) {
        ...
        const processDoc = (doc, objName, params, key, versionId, cb) => {
            if (!doc && params && params.versionId) {
        ...
        this.getBucketVFormat(bucketName, log, (err, _vFormat) => {
            ...
            return c.find({
                ...
                async.mapLimit(keysAndObjects, 5, ({ key, objName, params, versionId }, cb) => {
                    ...
                    processDoc(doc, objName, params, key, versionId, cb);
                }, callback);
            }).catch(err => {
                callback(err);
            });
     ...
     * @return {undefined}
     */
    async internalDeleteObject(collection, bucketName, key, params, filter, log, cb, originOp = 's3:ObjectRemoved:Delete') {
        const shouldDirectDelete = params.shouldOnlyDelete;
        // filter used when deleting object
        const deleteFilter = Object.assign({
            '_id': key,
        }, filter);
    
        if (shouldDirectDelete) {
            // If flag is true, directly delete object
            return collection.deleteOne(deleteFilter)
                .then(() => cb(null))
                .catch(err => {
                    log.error('internalDeleteObject: error deleting object', { bucket: bucketName, object: key, error: err.message });
                    return cb(errors.InternalError);
                });
        } 
