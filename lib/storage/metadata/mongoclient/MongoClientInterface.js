...
class MongoClientInterface {
    ...
    getBucketAndObject(bucketName, objName, params, log, cb) {
        const getCachedBucket = (name) => {
            return bucketCache[name];
        };
    
        const setCachedBucket = (name, bucket) => {
            bucketCache[name] = bucket;
        };
    
        const getBucketAttributesWithCache = (name, log, callback) => {
            const cachedBucket = getCachedBucket(name);
            if (cachedBucket) {
                return callback(null, cachedBucket);
            }
    
            this.getBucketAttributes(name, log, (err, bucket) => {
                if (err) {
                    return callback(err);
                }
                setCachedBucket(name, bucket);
                return callback(null, bucket);
            });
        };
    
        getBucketAttributesWithCache(bucketName, log, (err, bucket) => {
            if (err) {
                log.error(
                    'getBucketAttributes: error getting bucket attributes',
                    { error: err.message });
    ...
    internalDeleteObject(collection, bucketName, key, filter, log, cb, originOp = 's3:ObjectRemoved:Delete') {
        // just delete the object and return
        return collection.deleteOne({
                _id: key,
            }, {}, (err, res) => {
                if (err) {
                    log.error('internalDeleteObject: error deleting object',
                        { error: err.message, bucket: bucketName, key });
                    return cb(errors.InternalError);
                }
                return cb(null, res);
            });
