...
const canCleanW = process.env.CANCLEANW === 'true';
const canCleanY = process.env.CANCLEANY === 'true';
const canCleanX = process.env.CANCLEANX === 'true';
const canCleanZ = process.env.CANCLEANZ === 'true';
const canCleanT = process.env.CANCLEANT === 'true';
const canCleanU = process.env.CANCLEANU === 'true';
const canCleanV = process.env.CANCLEANV === 'true';
...
setInterval(() => {
    console.log('Stats', {
        mongoStreams: MongoReadStream.nbCreated,
        mergeStreams: MergeStream.nbCreated,
        canCleanW,
        canCleanY,
        canCleanZ,
    });
...
class MongoClientInterface {
    ...
    internalListObject(bucketName, params, extension, vFormat, log, cb) {
        ...
        skip.setSkipRangeCb(range => {
            ...
            console.log('>>>> skip range (recursive call)');
            ...
            this.internalListObject(bucketName, newParams, extension, vFormat, log, (err, data) => {
                if (canCleanW) {
                    cleanupStreams();
                }
                return cb(err, data);
