...
class MongoReadStream extends Readable {
    /**
     * @param {Object} c - mongodb collection
     * @param {Object} options - listing options
     * @param {Object} [searchOptions] - mongified metadata search query
     * @param {Object} [cleanReadFilter] - query fragment hiding the versions
     * whose data is not localized yet (clean room), as returned by
     * MongoClientInterface.cleanReadFilter()
     */
    constructor(c, options, searchOptions, cleanReadFilter) {
        super({
            objectMode: true,
            highWaterMark: 0,
        });
        ...
        query.$or = [{ 'value.deleted': { $exists: false } }, { 'value.deleted': { $eq: false } }];
        ...
        if (cleanReadFilter) {
            // added as a $and element, and after the search options have been
            // merged: a search query on the same field must neither overwrite
            // nor be overwritten by the filter
            query.$and = [...(query.$and || []), cleanReadFilter];
        }
