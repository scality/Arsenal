...
class MongoReadStream extends Readable {
     ...
     * @param {Object} [nonLocalizedFilter] - query fragment hiding the
     * non-localized versions, as returned by
     * MongoClientInterface.nonLocalizedFilter()
     */
    constructor(c, options, searchOptions, nonLocalizedFilter) {
        super({
            objectMode: true,
            highWaterMark: 0,
        });
        ...
        if (nonLocalizedFilter) {
            // as a $and element, after the search options are merged: a search
            // query on the same field must neither overwrite the filter nor be
            // overwritten by it
            query.$and = [...(query.$and || []), nonLocalizedFilter];
        }
