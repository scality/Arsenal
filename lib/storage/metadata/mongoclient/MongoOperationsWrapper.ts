...
export class MongoOperationsWrapper {
    ...
    private static execute<T>(
        ...
        if (!shouldBatch) {
            log.debug('Executing MongoDB operation directly (batching thresholds not met)', {
                operation,
                isWrite,
                collection: collection.collectionName,
    ...
    private shouldEnableBatching(isWrite: boolean): boolean {
        if (isWrite) {
            return this.config.enableWriteBatching && 
                   (this.config.writeOpsPerSecThreshold === 0 || 
                    this.writeOpsTracker.currentOpsPerSec >= this.config.writeOpsPerSecThreshold);
        } else {
            return this.config.enableReadBatching && 
                   (this.config.readOpsPerSecThreshold === 0 || 
                    this.readOpsTracker.currentOpsPerSec >= this.config.readOpsPerSecThreshold);
        }
    }
        ...
        try {
            ...
            collection.bulkWrite(bulkOps, { ordered: false })
                .then((result: BulkWriteResult) => {
                    log.debug('Bulk write completed successfully', {
                        collection: collection.collectionName,
