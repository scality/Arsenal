...
class MetadataWrapper {
    constructor(clientName, params, bucketclient, logger) {
        ...
        // silently ignoring the flag would expose the versions it must hide
        if (params.hideNonLocalizedVersions && this.implName !== 'mongoclient') {
            throw new Error(`hiding the non-localized versions is not supported by the ${this.implName} backend`);
        }
        this._hideNonLocalizedVersions = Boolean(params.hideNonLocalizedVersions);
    }
    ...
    _readParams(params) {
        ...
        return Object.assign({}, params, { hideNonLocalizedVersions: true });
    }
