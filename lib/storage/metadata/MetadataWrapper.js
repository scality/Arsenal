...
class MetadataWrapper {
    ...
    executeBatch(type, bucketName, ops, log, cb) {
        if (typeof this.client.executeBatch !== 'function') {
            log.debug('batch operation not suppored');
            return cb(errors.NotImplemented);
        }
        log.debug('executing batch operations');
        return this.client.executeBatch(type, bucketName, ops, log, cb);
    }
}
