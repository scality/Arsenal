...
function _parseListEntries(entries) {
    return entries.map(entry => {
        ...
        const userMetadata = Object.keys(tmp)
        ...
        if (tmp.archive) {
            restoreStatus = {
                inProgress: !!tmp.archive?.restoreRequestedAt && !tmp.archive.restoreCompletedAt,
                expiryDate: tmp.archive?.restoreWillExpireAt,
            };
        }
        ...
        return {
            ...
            value: {
                ...
                ChecksumValue: tmp.checksumValue,
...
class MetadataWrapper {
    constructor(clientName, params, bucketclient, logger) {
        ...
        const canHideNonLocalizedVersions = Boolean(this.client?.supportsHideNonLocalizedVersions);
        // ignoring the flag silently would expose the versions it must hide
        if (params.hideNonLocalizedVersions && !canHideNonLocalizedVersions) {
            logger.error('hiding the non-localized versions is not supported by the backend, ignoring the flag', {
                implName: this.implName,
            });
        }
        this._hideNonLocalizedVersions = Boolean(params.hideNonLocalizedVersions) && canHideNonLocalizedVersions;
    }

    _readParams(params) {
        if (!this._hideNonLocalizedVersions) {
            return params;
        }
        return Object.assign({ hideNonLocalizedVersions: true }, params);
    }
    ...
    getBucket(bucketName, log, cb) {
        ...
        this.client.getBucketAttributes(bucketName, log, (err, data, raftSessionId) => {
            ...
            return cb(err, BucketInfo.fromObj(data), raftSessionId);
        });
    }
    ...
    getObjectsMD(bucketName, objNamesWithParams, log, cb) {
        ...
        const objects = this._hideNonLocalizedVersions
            ? objNamesWithParams.map(({ key, params }) => ({ key, params: this._readParams(params) }))
            : objNamesWithParams;
    ...
    getObjectMD(bucketName, objName, params, log, cb) {
        ...
        this.client.getObject(bucketName, objName, this._readParams(params), log, (err, data) => {
            if (err) {
                log.debug('error from metadata', { implName: this.implName, err });
                return cb(err);
            }
    ...
    listObject(bucketName, listingParams, log, cb) {
        ...
        this.client.listObject(bucketName, this._readParams(listingParams), log, (err, data) => {
    ...
    listLifecycleObject(bucketName, listingParams, log, cb) {
        ...
        this.client.listLifecycleObject(bucketName, this._readParams(listingParams), log, (err, data) => {
            if (err) {
    ...
    listMultipartUploads(bucketName, listingParams, log, cb) {
        this.client.listMultipartUploads(bucketName, this._readParams(listingParams), log, (err, data) => {
