...
class MetadataWrapper {
    ...
    listObject(bucketName, listingParams, log, cb, oTel) {
        const {
            tracer,
            activeSpan,
            activeTracerContext,
        } = oTel;
        return tracer.startActiveSpan('listing from metadata', undefined, activeTracerContext, metadataSpan => {
            ...
            this.client.listObject(bucketName, listingParams, log, (err, data) => {
                ...
                activeSpan.addEvent('got data from Metadata');
                metadataSpan.end();
                if (err) {
                    log.debug('error from metadata', { implName: this.implName,
                        err });
