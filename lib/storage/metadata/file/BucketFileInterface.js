...
class BucketFileInterface {
     ...
     *  @param {Object} oTel - Object containing OpenTelemetry objects
     *  @return {undefined}
     */
    internalListObject(bucketName, params, log, cb, oTel) {
        const {
            tracer,
            activeSpan,
            activeTracerContext,
        } = oTel;
        return tracer.startActiveSpan('BucketFileInterface.internalListObject',
            undefined, activeTracerContext, internalListObjectSpan => {
                activeSpan.addEvent('entered Internal List Object');
                const extName = params.listingType;
                const extension = new list[extName](params, log);
                const requestParams = extension.genMDParams();
                this.loadDBIfExists(bucketName, log, (err, db) => {
                    activeSpan.addEvent('got bucket attributes from metadata');
                    if (err) {
                        return cb(err);
                    }
                    let cbDone = false;
                            ...
                            stream
                                ...
                                .on('end', () => {
                                    if (!cbDone) {
                                        ...
                                        activeSpan.addEvent('finished listing from metadata');
                                        internalListObjectSpan.end();
                                        cb(null, data);
                                    }
                                });
    ...
    listObject(bucketName, params, log, cb, oTel) {
        return this.internalListObject(bucketName, params, log, cb, oTel);
    }
