...
const { openSubLevel, createReadStream, createKeyStream } = require('./levelUtils');
...
class RecordLogProxy extends rpc.BaseClient {
    constructor(params) {
        ...
        this.addRequestInfoProducer(logProxy => ({ logName: logProxy.name }));
    }
}
...
class ListRecordStream extends stream.Transform {
    ...
    _transform(dbRecord, encoding, callback) {
        ...
        if (!this.hasStarted) {
            ...
            if (this.limit !== undefined && startSeq + this.limit <= this.endSeq) {
...
class RecordLogService extends rpc.BaseService {
     ...
     * @param {LevelDB} params.rootDb - root LevelDB metadata database object
            ...
            if (reqParams.logName) {
                ...
                if (!openLog) {
                    openLog = {
                        ...
                        logDb: openSubLevel(this.rootDb, [RECORD_LOG_PREFIX, reqParams.logName]),
        ...
        this.registerAsyncAPI({
            ...
            readRecords: (env, params, cb) => {
                ...
                this._findNextSeq(openLog, err => {
                    ...
                    // abstract-level encodes unset bounds as an 'undefined'
                    // key instead of ignoring them
                    const queryParams = {};
                    if (_params.startSeq) {
                        queryParams.gte = formatSeq(_params.startSeq);
                    }
                    if (endSeq) {
                        queryParams.lte = formatSeq(endSeq);
                    }
                    if (_params.limit !== undefined) {
                        queryParams.limit = _params.limit;
                    }
                    const userStream = new ListRecordStream(endSeq, _params.limit);
                    const dbStream = createReadStream(openLog.logDb, queryParams);
                    dbStream.pipe(userStream);
                    dbStream.once('error', err => userStream.emit('error', err));
    ...
    _findNextSeq(openLog, cb) {
        ...
        let lastSeq;
        return createKeyStream(openLog.logDb, { reverse: true, limit: 1 })
            .on('data', key => {
