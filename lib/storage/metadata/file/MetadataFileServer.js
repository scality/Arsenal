...
const { openSubLevel, createReadStream, createKeyStream, resolveBatchSubLevels } = require('./levelUtils');
const { RecordLogService } = require('./RecordLog.js');

const WGM = require('../../../versioning/WriteGatheringManager').default;
const WriteCache = require('../../../versioning/WriteCache').default;
const VRP = require('../../../versioning/VersioningRequestProcessor').default;
...
class MetadataFileServer {
    ...
    initMetadataService() {
        ...
        this.rootDb = new ClassicLevel(`${this.path}/${ROOT_DB}`);
        const dbService = new levelNet.LevelDbService({
            rootDb: this.rootDb,
            lookupSubLevel: path => openSubLevel(this.rootDb, path),
            namespace,
            logger: this.logger,
        });
        ...
        const metadataAPI = {
            get: (request, logger, callback) => {
                ...
                subDb.get(request.key).then(
                    data => callback(data === undefined ? errors.ObjNotFound : null, data),
                    err => callback(err),
                );
            },
            ...
            batch: (request, logger, callback) => {
                const dbPath = request.db.split(SUBLEVEL_SEP);
                const ops = request.array.map(op => Object.assign({}, op, { prefix: dbPath }));
                const commit = batch =>
                    this.rootDb.batch(resolveBatchSubLevels(this.rootDb, batch), SYNC_OPTIONS).then(
                        () => callback(),
                        err => callback(err),
                    );
                if (this.recordLog.enabled) {
                    this.recordLogService
                        .withRequestParams({ logName: this.recordLog.recordLogName })
                        .createLogRecordOps(ops, (err, logEntries) => {
                            if (err) {
                                return callback(err);
                            }
                            debug('ops to batch:', ops);
                            debug('log entries:', logEntries);
                            return commit(ops.concat(logEntries));
                        });
                } else {
                    commit(ops);
                }
        ...
        dbService.registerAsyncAPI({
            put: (env, key, value, options, cb) => {
                ...
                vrp.put({ db: dbName, key, value, options }, env.requestLogger, cb);
            },
            del: (env, key, options, cb) => {
                const dbName = env.subLevel.join(SUBLEVEL_SEP);
                vrp.del({ db: dbName, key, options }, env.requestLogger, cb);
            },
            get: (env, key, options, cb) => {
                const dbName = env.subLevel.join(SUBLEVEL_SEP);
                vrp.get({ db: dbName, key, options }, env.requestLogger, cb);
            },
            getDiskUsage: (env, cb) => storageUtils.getDiskUsage(this.path, cb),
        });
        dbService.registerSyncAPI({
            createReadStream: (env, options) => createReadStream(env.subDb, options),
            rawListKeys: (env, options) => createKeyStream(this.rootDb, options),
