const { parseRequestTarget } = require('../../../utils/requestUrl');
...
function getURIComponents(uri, logger) {
    try {
        ...
        const { pathname, query } = parseRequestTarget(uri);
        ...
        logger.error('Invalid URI: failed to parse', { uri, error: ex, errorStack: ex.stack, message: ex.message });
        return null;
    }
}
