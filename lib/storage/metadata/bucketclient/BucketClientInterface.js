...
class BucketClientInterface {
    ...
    deleteObject(bucketName, objName, params, log, cb, originOp) {
        if (originOp) {
            if (!params) {
                params = {};
            }
            params.originOp = originOp;
            if (params.overheadField) {
                // new array as params.overheadField is a reused constant from cloudserver
                params.overheadField = params.overheadField.concat(['originOp']);
            } else {
