...
function retrieveData(locations, dataRetrievalFn,
    response, logger, extraParams, errorHandlerFn) {
    ...
    return dataRetrievalFn(current, logger, extraParams,
        (err, readable) => {
            if (err) {
            ...
            readable.on('end', () => {
                ...
                dataRetrievalFn, response, logger, extraParams);
            });
...
const routesUtils = {
     ...
     * @param {object} log - Werelogs logger
     * @param {object} [extraParams] - object containing extra params
     * @param {strinig} [extraParams.bucketName] - bucket name if needed
     * for streaming
     * @return {undefined}
     */
    responseStreamData(errCode, overrideParams, resHeaders, dataLocations,
        dataRetrievalFn, response, range, log, extraParams) {
        if (errCode && !response.headersSent) {
            return XMLResponseBackend.errorResponse(errCode, response, log,
                                                    resHeaders);
        }
