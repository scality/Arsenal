...
function retrieveData(locations, dataRetrievalFn,
    response, logger, streamingParams, errorHandlerFn) {
    ...
    return dataRetrievalFn(current, logger, streamingParams,
        (err, readable) => {
            if (err) {
            ...
            readable.on('end', () => {
                process.nextTick(retrieveData, locations,
                dataRetrievalFn, response, logger, streamingParams);
            });
...
const routesUtils = {
     ...
     * @param {object} log - Werelogs logger
     * @param {object} [streamingParams] - object containing any additional
     *   params needed for streaming from the data backend
     * @param {string} [streamingParams.bucketName] - bucket name if needed
     * for streaming
     * @return {undefined}
     */
    responseStreamData(errCode, overrideParams, resHeaders, dataLocations,
        dataRetrievalFn, response, range, log, streamingParams) {
        if (errCode && !response.headersSent) {
            return XMLResponseBackend.errorResponse(errCode, response, log,
                                                    resHeaders);
        }
        ...
        return retrieveData(dataLocations, dataRetrievalFn,
            response, log, streamingParams);
    },
