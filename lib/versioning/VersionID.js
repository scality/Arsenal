...
function base62Encode(str) {
    ...
    const enc3String = enc3.toString();
    const totalLength = padded1.toString().length +
        padded2.toString().length +
        enc3String.length;
    const enc3PaddingNeeded = 32 - totalLength + enc3String.length;

    return padded1 + padded2 + enc3.toString().padEnd(enc3PaddingNeeded, '\u0000');
}
...
function base62Decode(str) {
    try {
        ...
        // take off null bytes padding only from the end
        const re = new RegExp('\u0000+$'); // eslint-disable-line
        enc3 = enc3.replace(re, '');
