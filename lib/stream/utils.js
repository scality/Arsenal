        ...
        s.on('end', () => {
            ...
            try {
                ...
                if (joiSchema) {
                    ...
                    validatedContents = value;
                } else {
                    validatedContents = parsedContents;
                }
                resolve(validatedContents);
            } catch (err) {
                reject(errors.InvalidArgument.customizeDescription(`invalid input: ${err.message}`));
            }
        });
        s.once('error', reject);
    });
