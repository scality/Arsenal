...
const canCleanM = process.env.CANCLEANM === 'true';
    ...
    _destroy(err, callback) {
        ...
        for (let i = 0; i < 2; ++i) {
            if (!this._streamEof[i]) {
                ...
                if (this._streams[i] && canCleanM) {
                    console.log('>>> cleaning stream listeners');
                    this._streams[i].removeAllListeners();
                }
            }
        }
        if (canCleanM) {
