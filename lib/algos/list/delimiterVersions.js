...
class DelimiterVersions extends Delimiter {
    ...
    filterV0(obj) {
        ...
        return this.addContents({ key: nonversionedKey,
                                  value: obj.value, versionId });
    }
    ...
    filterV1(obj) {
        ...
        const versionIdIndex = key.indexOf(VID_SEP);
        const nonversionedKey = key.slice(0, versionIdIndex);
        const versionId = key.slice(versionIdIndex + 1);
        if (this.delimiter) {
            const baseIndex = this.prefix ? this.prefix.length : 0;
            const delimiterIndex =
                  nonversionedKey.indexOf(this.delimiter, baseIndex);
