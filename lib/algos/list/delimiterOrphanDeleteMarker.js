...
class DelimiterOrphanDeleteMarker extends DelimiterVersions {
    ...
    constructor(parameters, logger, vFormat) {
        const { marker, maxKeys, prefix, beforeDate, maxScannedLifecycleListingEntries } = parameters;
        ...
        this.keyName = null;
        this.value = null;
        // Key of a PHD master sitting in this.keyName: a placeholder, not a real entry.
        this.phdKeyName = null;
        this.scannedKeys = 0;
    }
    ...
    addVersion(key, versionId, value) {
        ...
        // Reached only when this key's master is a PHD. A PHD is never
        // deduplicated (its versionId is generated at delete time, matching no
        // version key), and handlePHDMaster() already put the key in keyName.
        // So this version is the key's first real entry, not its second: it
        // stands in for the master, and the repair job will promote it there.
        //
        // Without this branch it falls through and gets read as "this key was
        // already seen, so it has more than one entry and cannot be an orphan".
        // Which is true of keyName, but the PHD filled it, not an entry.
        //
        // Park it, and leave prevKeyName alone: handlePHDMaster already moved
        // it, and this key is still undecided. The verdict comes later, when an
        // entry for a different key arrives: whatever is still parked was this
        // key's only entry, and a lone delete marker is an orphan.
        //
        //   cat.jpg      PHD           -> keyName='cat.jpg', phdKeyName='cat.jpg'
        //   cat.jpg\0vB  delete marker -> parked here
        //   dog.jpg      different key -> cat.jpg was alone -> published
        if (this.phdKeyName === key) {
            this.phdKeyName = null;
            this.value = value;
    ...
    /**
     * Process a scanned PHD master key as a key transition, by delegating to
     * addVersion() with a null value. This serves two purposes:
     *
     ...
     *
     * The PHD key itself is never held as a candidate (value = null): a
     * placeholder is not a delete marker. But the slot is tagged with
     * phdKeyName, so addVersion() still holds the first version scanned for
     * that key. A PHD master keeps its surviving versions until the repair job
     * promotes one back to master, and a lone surviving delete marker under it
     * is an orphan that must be listed.
     *  @param {String} key   - The PHD master key
     *  @return {number} - filter return value
     */
    handlePHDMaster(key) {
        this.addVersion(key, null, null); // null value: never a candidate
        this.phdKeyName = key; // after the call, which clears it
