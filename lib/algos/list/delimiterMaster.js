...
const { FILTER_ACCEPT, FILTER_SKIP, SKIP_NONE, FILTER_END } = require('./tools');
...
class DelimiterMaster extends Delimiter {
    ...
    addCommonPrefix(key, index) {
        const commonPrefix = this.getCommonPrefix(key, this.delimiter, index);
    ...
    skippingBase() {
        if (this[this.nextContinueMarker]) {
            ...
            // console.log('inside skippingBase', { index, continueMarker: this[this.nextContinueMarker] });
