...
class DelimiterMaster extends Delimiter {
    ...
    filterV0(obj) {
        ...
        if ((this.prefix && !key.startsWith(this.prefix))
            ...
            console.log('Skip keys not starting with the prefix or not alphabeticall', {obj, prefix: this.prefix, marker: this[this.nextContinueMarker] });
            return FILTER_SKIP;
        }
        ...
        if (versionIdIndex >= 0) {
             ...
             *   */
            // console.log({ skipBecauseDelimiterAndStartwith: this.delimiter &&
            //     key.startsWith(this[this.nextContinueMarker]), key, continueMarker: this[this.nextContinueMarker] });
            if (key === this.prvKey || key === this[this.nextContinueMarker]) { // skips because of this..
                /* master version already filtered */
                console.log('skips because of key === this.prvKey || key === this[this.nextContinueMarker]', 
                {key, continueMarker: this[this.nextContinueMarker]});
                return FILTER_SKIP;
            }

            if (this.delimiter && key.startsWith(this[this.nextContinueMarker])) {
                // if this[continueMarker] ends with the delimiter then we have to treat it differently

                // blah/    -> blah/ v12 (skip)   -> key=blah/             :(split on / -> [blah]) -> len 1 so skip
                // blah/    -> blah/goo v23 (dont skip) -> key=blah/goo    :(split on / -> [blah, goo]) -> len>1 accept
                // only skip in case 1 and let addContents handle case 2?
                const candidate = key.split(this.delimiter);
                const len = candidate.length;
                if (len === 1) {
                    // case key=foo: this is a version of a previously seen master key.
                    console.log('len ==1: case key=foo: this is a version of a previously seen master key.', {key, candidate});
                    return FILTER_SKIP;
                }
                ...
                if (len > 1) {
                    ...
                    if (key[len - 1] === '') {
                        ...
                        console.log('len > 1: key=foo/ : version of a previously seen master key.', {key, candidate});
                        return FILTER_SKIP;
                    }
                    // else key=foo/bar (or further nesting as foo/bar/baz/..)
                    // This case should not be skipped and be handled in this.addContents below.
                }
            }
        }
