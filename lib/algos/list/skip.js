...
class Skip {
    ...
    filter(entry) {
        ...
        // Check if extension says to end (maxKeys reached)
        if (filteringResult === FILTER_END) {
            console.log(`[SKIP_DEBUG] FILTER_END reached from extension (maxKeys reached), calling listingEndCb`);
            this.listingEndCb();
            ...
            if (++this.streakLength >= MAX_STREAK_LENGTH) {
                ...
                const rangesEqual = this._areRangesEqual(newRange, this.gteParams);
