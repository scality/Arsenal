...
// delay to post statement if session starting up
const POST_STATEMENT_DELAY = 5000;
// delay to try getting statement result if not ready
const GET_STATEMENT_DELAY = 8000;
...
class LivyClient {
     ...
     * @return {undefined}
     */
    postStatement(sessionId, codeToExecute, callback) {
        assert(typeof callback === 'function', 'callback must be a function');
        assert(Number.isInteger(sessionId), 'sessionId must be an integer');
        ...
        this.getSessionOrBatchState('session', sessionId, (err, res) => {
            ...
            if (res.state === 'starting' || res.state === 'busy') {
                ...
                return setTimeout(this.postStatement.bind(this),
                    POST_STATEMENT_DELAY, sessionId, codeToExecute, callback);
            }
            if (res.state !== 'idle') {
     ...
     * @return {undefined}
     */
    getStatement(sessionId, statementId, callback) {
        assert(typeof callback === 'function', 'callback must be a function');
        assert(Number.isInteger(sessionId), 'sessionId must be an integer');
        ...
        this._request('GET', `/sessions/${sessionId}/statements/${statementId}`,
            null, null, (err, res) => {
                ...
                if (res && res.output && res.output.data) {
                ...
                return setTimeout(this.getStatement.bind(this),
                    GET_STATEMENT_DELAY, sessionId, statementId, callback);
            });
        return undefined;
    }
