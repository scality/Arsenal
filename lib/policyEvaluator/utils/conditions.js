...
/**
 * checkIPinRange checks whether a given ip address is in an ip address
 * range
 * @param {string} item - ip address range
 * @param {object} key - parsed ip address
 * @return {boolean} true if in range, false if not
 */
function checkIPinRange(item, key) {
    let range;

    try {
        range = ipAddr.IPv4.parseCIDR(item);
    } catch (err) {
        try {
        // not ipv4 so try ipv6
            range = ipAddr.IPv6.parseCIDR(item);
        } catch (err) {
...
conditions.convertConditionOperator = operator => {
    ...
    const operatorMap = {
        ...
        IpAddress: function ipAddress(key, value) {
            return value.some(item => checkIPinRange(item, key));
        },
