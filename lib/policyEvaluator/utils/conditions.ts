 ...
 */
export function findConditionKey(key: string, requestContext: RequestContext): any {
    ...
    switch (key) {
        ...
        case 'aws:TokenIssueTime':
            return requestContext.getTokenIssueTime();
        ...
        case 'aws:MultiFactorAuthPresent':
            return requestContext.getMultiFactorAuthPresent();
        ...
        case 'aws:MultiFactorAuthAge':
            return requestContext.getMultiFactorAuthAge();
        ...
        case 'aws:principaltype':
            return requesterInfo.principaltype;
        ...
        case 'aws:SecureTransport':
            return requestContext.getSslEnabled() ? 'true' : 'false';
        ...
        case 'aws:SourceIp':
            return requestContext.getRequesterIp();
        ...
        case 's3:x-amz-grant-read':
            return headers['x-amz-grant-read'];
        case 's3:x-amz-grant-write':
            return headers['x-amz-grant-write'];
        case 's3:x-amz-grant-read-acp':
            return headers['x-amz-grant-read-acp'];
        case 's3:x-amz-grant-write-acp':
            return headers['x-amz-grant-write-acp'];
        case 's3:x-amz-grant-full-control':
            return headers['x-amz-grant-full-control'];
        // s3:x-amz-copy-source is x-amz-copy-source header if applicable on
        // a put object
        case 's3:x-amz-copy-source':
        ...
        case 's3:x-amz-metadata-directive':
            return headers['metadata-directive'];
        ...
        case 's3:signatureAge':
            return requestContext.getSignatureAge();
        ...
        case 's3:x-amz-content-sha256':
            return headers['x-amz-content-sha256'];
        ...
        // header
        case 's3:ObjLocationConstraint':
            return headers['x-amz-meta-scal-location-constraint'];
        case 'sts:ExternalId':
            return requestContext.getRequesterExternalId();
        case 'keycloak:groups':
            return requesterInfo.keycloakGroup;
        case 'keycloak:roles':
            return requesterInfo.keycloakRole;
        case 'iam:PolicyArn':
            return requestContext.getPolicyArn();
        ...
        case 's3:ExistingObjectTag':
            return requestContext.getNeedTagEval() ? requestContext.getExistingObjTag() : undefined;
        ...
        case 's3:RequestObjectTagKey':
            return requestContext.getNeedTagEval() ? requestContext.getRequestObjTags() : undefined;
...
function convertSpecialChars(string: string) {
    ...
    return string.replace(/(\$\{\*\})|(\$\{\?\})|(\$\{\$\})/g, characterMap);
}
...
export function convertConditionOperator(operator: string): boolean {
    ...
    const operatorMap = {
        ...
        StringNotEqualsIgnoreCase: function stringNotEqualsIgnoreCase(key: string, value: string[]) {
        ...
        ArnLike: function ArnLike(key: string, value: string[]) {
            ...
            return value.some(policyArn => checkArnMatch(policyArn, requestRelativeId, requestArnArr));
        },
        ...
        Null: function nullOperator(key: string, value: string[]) {
            ...
            // not null).
            if (
                ((key === undefined || key === null) && value[0] === 'true') ||
                (key !== undefined && key !== null && value[0] === 'false')
            ) {
                return true;
            }
            return false;
