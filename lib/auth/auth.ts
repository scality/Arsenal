...
function generateV4Headers(
    ...
    const currentHeaders = request.getHeaders ? request.getHeaders() : request._headers || {};
    request.setHeader('host', currentHeaders.host);
    ...
    Object.assign(request.headers, request.getHeaders ? request.getHeaders() : request._headers || {});
    const signedHeaders = Object.keys(request.getHeaders ? request.getHeaders() : request._headers || {})
        .filter(headerName =>
