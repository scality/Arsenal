...
function extractQueryParams(queryObj, log) {
    ...
    if (credential && credential.length > 28 && credential.indexOf('/') > -1) {
        ...
        // in case access keys contains a '/', reduce the credentials array to
        // the desire size by recomputing the access key. This step is based on
        // fact than no other array component will have a '/' character.
        while (authParams.credential.length > 5) {
            authParams.credential[0] += `/${authParams.credential[1]}`;
            authParams.credential.splice(1, 1);
        }
    } else {
...
function extractAuthItems(authHeader, log) {
    ...
    if (credentialStr && credentialStr.trim().startsWith('Credential=')
        ...
        // the desire size by recomputing the access key. This step is based on
        // fact than no other array component will have a '/' character.
        while (authItems.credentialsArr.length > 5) {
            authItems.credentialsArr[0] += `/${authItems.credentialsArr[1]}`;
            authItems.credentialsArr.splice(1, 1);
        }
        log.trace('resulting credentials', { arr: authItems.credentialsArr });
    } else {
