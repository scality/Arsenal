...
function check(request, log, data) {
    ...
    */
    ...
    const preSignUrlExpiry = process.env.PRE_SIGN_URL_EXPIRY && !Number.isNaN(process.env.PRE_SIGN_URL_EXPIRY)
        ? Number.parseInt(process.env.PRE_SIGN_URL_EXPIRY) : 604800000;
    // 604800000 ms (seven days).
    if (expirationTime > currentTime + preSignUrlExpiry) {
