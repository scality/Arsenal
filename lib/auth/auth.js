...
function generateV4Headers(request, data, accessKey, secretKeyValue,
    ...
    if (request.method === 'POST') {
        const payload = queryString.stringify(data, null, null, {
            encodeURIComponent,
        });
        ...
        if (!cacheNoPayload) {
            cacheNoPayload = crypto.createHash('sha256')
                .update('', 'binary').digest('hex');
        }
        payloadChecksum = cacheNoPayload;
    }
    const rawHeaders = request._headers;
    const headers = request.headers;
    headers['x-amz-date'] = amzDate;
    headers['x-amz-content-sha256'] = payloadChecksum;
    const rawHeadersLen = rawHeaders.length;
    const signedHeaders = [ host ];
    for (let i = 0; i < rawHeadersLen; ++i) {
        const name = rawHeaders[i];
        headers[name] = rawHeaders[name];
        if (name.startsWith('x-amz-') || name.startsWith('x-scal-')) {
    ...
    // Object.keys(request._headers)
    //     .filter(headerName =>
    //         headerName.startsWith('x-amz-')
    //         || headerName.startsWith('x-scal-')
    //         || headerName === 'host'
    //     ).sort().join(';');
    const params = { request, signedHeadersStr, payloadChecksum,
