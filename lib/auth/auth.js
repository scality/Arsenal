...
function extractParams(request, log, awsService, data) {
    ...
    Object.keys(modifiedRequest.headers).forEach(headerName => {
        if (headerName.startsWith('x-amz-meta-scality-rewritten-')) {
            const decodedHeader = decodeHeader(headerName);
            modifiedRequest.headers[decodedHeader] = request[headerName];
            delete modifiedRequest[headerName];
        }
    });
