...
class ObjectMD {
    ...
    _initMd() {
        ...
        this._data = {
            ...
            'key': '',
            'location': null,
            'nonTransientLocation': undefined,
            'isNull': '',
            'nullVersionId': '',
            'isDeleteMarker': '',
    ...
    /**
     * Define the preferred non-transient location for objects on a
     * transient source
     *
     * @param {undefined|string|object[]} nonTransientLocation -
     * undefined if current location is not transient, 'PENDING' if
     * CRR to non-transient location is pending, or an array of
     * locations for the non-transient target.
     * @return {ObjectMD} itself
     */
    setNonTransientLocation(nonTransientLocation) {
        this._data.nonTransientLocation = nonTransientLocation;
        return this;
    }

    /**
     * Returns the preferred non-transient location if object is on a
     * transient source and has been already CRR-ed to the preferred
     * location
     *
     * @return {undefined|string|object[]} non-transient location
     * array - undefined if current location is not transient,
     * 'PENDING' if CRR to non-transient location is pending, or an
     * array of locations for the non-transient target.
     */
    getNonTransientLocation() {
        return this._data.nonTransientLocation;
    }
