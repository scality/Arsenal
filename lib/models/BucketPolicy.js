...
class BucketPolicy {
     ...
     */
    getBucketPolicy() {
        const policy = this._getPolicyJSON();
        return policy;
    }
    ...
    _getPolicyJSON() {
        ...
        if (!this._json || this._json === '') {
            policy.error = errors.MalformedPolicy.customizeDescription(
                'request json is empty or undefined');
            return policy;
        }
        const validSchema = validateResourcePolicy(this._json);
        if (validSchema.error) {
            policy.error = validSchema.error;
            return policy;
        }
        this._getStatementArray();
        const valAcRes = this._validateActionResource();
        if (valAcRes.error) {
            policy.error = valAcRes.error;
            return policy;
        }
    ...
    _validateActionResource() {
        const obj = {};
        this._policy.Statement.forEach(s => {
            ...
            const allAction = actions.some(a => a.includes('*'));
            const objectResource = resources.some(r => r.includes('/'));
            if ((objectAction && !objectResource) ||
            (objectResource && !objectAction && !allAction)) {
                obj.error = errors.MalformedPolicy.customizeDescription(
    ...
    static validatePolicy(policy) {
        return validateResourcePolicy(policy);
    }
}
