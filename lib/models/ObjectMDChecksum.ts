...
export default class ObjectMDChecksum {
    ...
    constructor(
        ...
        if (!(CHECKSUM_ALGORITHMS as readonly string[]).includes(checksumAlgorithm)) {
            throw new Error(`invalid checksumAlgorithm: ${checksumAlgorithm}`);
        }
        if (!(CHECKSUM_TYPES as readonly string[]).includes(checksumType)) {
            throw new Error(`invalid checksumType: ${checksumType}`);
        }
        if (!isValidDigest(checksumAlgorithm, checksumValue)) {
            throw new Error(`invalid checksumValue for ${checksumAlgorithm}: ${checksumValue}`);
        }
    ...
    static isValid(data: {
        ...
        try {
            new ObjectMDChecksum(data.checksumAlgorithm, data.checksumValue, data.checksumType);
            return true;
        } catch {
            return false;
        }
