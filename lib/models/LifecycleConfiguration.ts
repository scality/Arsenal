...
export default class LifecycleConfiguration {
    ...
    _buildRulesArray() {
        ...
        if (!this._parsedXML.LifecycleConfiguration && this._parsedXML.LifecycleConfiguration !== '') {
    ...
    _parseFilter(filter: any) {
        ...
        if ((filter.And && (filter.Prefix || filter.Tag)) || (filter.Prefix && filter.Tag)) {
    ...
    _parseTags(tags: { Key?: string; Value?: any[] }[]) {
        ...
        for (const tag of tags) {
            ...
            if (this._tagKeys.includes(tag.Key[0])) {
                tagObj.error = errorInstances.InvalidRequest.customizeDescription('Tag Keys must be unique');
                break;
            }
    ...
    _parseID(id: string[]) {
        // @ts-ignore
        const idObj: { error: ArsenalError; propName: 'ruleID'; ruleID?: any } | { propName: 'ruleID'; ruleID: any } = {
            propName: 'ruleID',
        };
    ...
    _checkStorageClasses(params: {
        ...
        if (usedStorageClasses.includes(storageClass)) {
            const msg =
                `'StorageClass' must be different for '${ancestor}' ` +
    ...
    _checkTimeGap(params: {
        ...
        if (invalidTransition) {
            ...
            const msg =
                `'${timeType}' in the 'Transition' action for ` +
    ...
    _checkTimeType(params: {
        ...
        if (rule.Expiration && rule.Expiration[0][currentTimeType] === undefined) {
            const msg =
     ...
     * }
     */
    _parseNoncurrentVersionTransition(rule: { NoncurrentVersionTransition: any[]; Prefix?: string[]; Filter?: any }) {
        ...
        for (let i = 0; i < rule.NoncurrentVersionTransition.length; i++) {
            ...
            const noncurrentDays: number | undefined = t.NoncurrentDays && Number.parseInt(t.NoncurrentDays[0], 10);
     ...
     * }
     */
    _parseTransition(rule: { Transition: any[]; Prefix?: string[]; Filter?: any }) {
        const transition: ({ days: number; storageClass: string } | { date: string; storageClass: string })[] = [];
        const usedStorageClasses: string[] = [];
     ...
     * }
     */
    _parseExpiration(rule: { Expiration: any[]; Filter?: any[] }) {
        const expObj: {
            error?: ArsenalError;
            days?: number;
            date?: number;
        ...
        if ((subExp.Date && (subExp.Days || subExp[eodm])) || (subExp.Days && subExp[eodm])) {
     ...
     * }
     */
    _parseNoncurrentVersionExpiration(rule: { NoncurrentVersionExpiration: any[] }) {
        ...
        rules.forEach((rule: any) => {
            ...
            actions.forEach((a: any) => {
                ...
                if (a.nonCurrentVersionTransition) {
                    assert.strictEqual(typeof a.nonCurrentVersionTransition, 'object');
    ...
    static getConfigXml(config: { rules: Rule[] }) {
        ...
        const rulesXML = rules
            .map(rule => {
                ...
                const Prefix = rulePrefix !== undefined ? `<Prefix>${escapeForXml(rulePrefix)}</Prefix>` : '';
                let tagXML = '';
                if (tags) {
                    tagXML = tags
                        .map(t => {
                            const { key, val } = t;
                            const Tag =
                                `<Tag><Key>${escapeForXml(key)}</Key>` + `<Value>${escapeForXml(val)}</Value></Tag>`;
                            return Tag;
                        })
                        .join('');
                }
                ...
                } else if (tags && (filter.rulePrefix !== undefined || tags.length > 1)) {
                ...
                const Actions = actions
                    .map(action => {
                        ...
                        if (actionName === 'AbortIncompleteMultipartUpload') {
                            Action =
                                `<${actionName}><DaysAfterInitiation>${days}` +
                            ...
                            const DelMarker = deleteMarker
                                ...
                                : '';
                            Action = `<${actionName}>${Days}${Date}${DelMarker}` + `</${actionName}>`;
                        }
