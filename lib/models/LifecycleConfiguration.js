...
class LifecycleConfiguration {
    ...
    /**
     * Parses the top-level prefix of the config
    ...
    _parseRule(rule) {
        ...
        if (rule.Prefix) {
            prefix = this._parsePrefix(rule.Prefix[0]);
            rulePropArray.push(prefix);
        }
        if (rule.Filter) {
            filter = this._parseFilter(rule.Filter[0]);
    ...
    static getConfigXml(config) {
        ...
        const rulesXML = rules.map(rule => {
            ...
            const Prefix = `<Prefix>${rulePrefix}</Prefix>`;
            let tagXML = '';
            if (tags) {
                tagXML = tags.map(t => {
                    const { key, val } = t;
