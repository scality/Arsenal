...
azureMpuUtils.putSinglePart = (azureClient, request, params, dataStoreName,
log, cb) => {
    ...
    return azureClient.createBlockFromStream(blockId,
...
azureMpuUtils.putNextSubPart = (azureClient, partParams, subPartInfo,
subPartStream, subPartIndex, resultsCollector) => {
    ...
    azureClient.createBlockFromStream(subPartId, bucketName,
    objectKey, subPartStream, subPartSize, {}, err => {
        resultsCollector.pushResult(err, subPartIndex);
    });
};

azureMpuUtils.putSubParts = (azureClient, request, params, dataStoreName,
log, cb) => {
    ...
    azureMpuUtils.putNextSubPart(azureClient, params, subPartInfo,
        currentStream, 0, resultsCollector);
    ...
    hashedStream.on('data', data => {
        ...
        if (currentLength + data.length > azureMpuUtils.maxSubPartSize) {
            ...
            azureMpuUtils.putNextSubPart(azureClient, params, subPartInfo,
                nextStream, subPartIndex, resultsCollector);
