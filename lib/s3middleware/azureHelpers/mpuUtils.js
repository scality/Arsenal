...
azureMpuUtils.putSinglePart = (that, request, params,
dataStoreName,
log, cb) => {
    ...
    that._errorWrapper.bind(that);
    that._errorWrapper('uploadPart', 'createBlockFromStream',
...
azureMpuUtils.putNextSubPart = (that, partParams, subPartInfo,
...
azureMpuUtils.putSubParts = (that, request, params,
dataStoreName, log, cb) => {
    ...
    azureMpuUtils.putNextSubPart(that, params, subPartInfo,
    ...
    hashedStream.on('data', data => {
        ...
        if (currentLength + data.length > azureMpuUtils.maxSubPartSize) {
            ...
            azureMpuUtils.putNextSubPart(that, params,
