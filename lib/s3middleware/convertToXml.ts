...
const convertMethods = {
    completeMultipartUpload: (xmlParams) => {
    ...
    initiateMultipartUpload: (xmlParams) => `
        <?xml version="1.0" encoding="UTF-8"?>
    ...
    listMultipartUploads: (xmlParams) => {
        const xml = [];
        ...
        const params = [
            ...
            {
                tag: 'NextUploadIdMarker',
                value: l.NextUploadIdMarker,
                optional: true,
            },
        ...
        params.forEach((param) => {
            if (param.value) {
                xml.push(
                    `<${param.tag}>${escapeForXml(param.value)}` +
                        `</${param.tag}>`
                );
...
export default function convertToXml(method: keyof typeof convertMethods, xmlParams) {
