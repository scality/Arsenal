...
class ProbeServer extends httpServer {
     ...
     *
     * @param {ProbeServerParams} params
     */
    constructor(params) {
         ...
         * @type {Object.<string, ProbeDelegate>}
         */
        this._handlers = {};
    }
     ...
     *     res.end()
     * })
     * @param {string|string[]} paths - URL path(s) for where the request should be handled
     * @param {ProbeDelegate} handler - Callback to handle request
     */
    addHandler(paths, handler) {
        if (typeof paths === 'string') {
            paths = [paths];
        }
        for (const p of paths) {
            this._handlers[p] = handler;
        }
    }

    /**
     * _onRequest is the internal handler for all HTTP requests
     *
    ...
    _onRequest(req, res) {
        ...
        if (!(req.url in this._handlers)) {
            errors.InvalidURI.writeResponse(res)
            return
        }

        const probeResponse = this._handlers[req.url](res, log);
